...
class GerenciadorSessoes:
    def __init__(self):
        ...
        self.conexão: Optional[sqlite3.Connection] = None
        self._lock = threading.RLock()

        self._inicializar_db()
        self.logger.info("[SESSES] Gerenciador de Sesses iniciado.DB: %s", self.db_path)

    def _inicializar_db(self) -> None:
        try:
            self.conexão = sqlite3.connect(str(self.db_path), check_same_thread=False)
            try:
                self.conexão.execute("PRAGMA foreign_keys = ON;")
            except Exception:
                pass

            cursor = self.conexão.cursor()
            cursor.execute("""
            ...
            """)
            self.conexão.commit()
            self.logger.info("[SESSES] Banco de dados inicializado.")
        except Exception as e:
            self.logger.critical("[SESSES] Erro ao inicializar DB %s: %s", self.db_path, e, exc_info=True)
    ...
    def obter_sessao(self, user_id: str, personalidade: str, tema: str, resumivel: bool = False) -> str:
        if not user_id or not personalidade or not tema:
            self.logger.error("[SESSES] Parmetros invlidos para obter_sessao.")
            raise ValueError("user_id, personalidade e tema so obrigatrios")

        personalidade_lower = personalidade.lower()
        if personalidade_lower not in self.almas_nomes:
            self.logger.warning("[SESSES] Personalidade desconhecida: %s", personalidade)
            raise ValueError(f"Personalidade invlida: {personalidade}. Escolha entre {', '.join(self.almas_nomes)}.")
        ...
        with self._lock:
            try:
                ...
                if not cursor.fetchone():
                    ...
                    self.logger.info("[SESSES] Nova sessão criada: %s (Alma=%s, Tema=%s)", sessao_id[:8], personalidade_lower, tema.lower())
                else:
                    cursor.execute(
                        "UPDATE sessoes SET data_ultimo_acesso = ? WHERE sessao_id = ?",
                        (datetime.datetime.now().isoformat(), sessao_id)
                    )
                    self.logger.info("[SESSES] Sessão retomada: %s (Alma=%s, Tema=%s)", sessao_id[:8], personalidade_lower, tema.lower())

                self.conexão.commit()
                return sessao_id
            except Exception as e:
                self.logger.error("[SESSES] Erro ao obter/criar sessão %s: %s", sessao_id[:8], e, exc_info=True)
    ...
    def obter_sessao_dados(self, sessao_id: str) -> Dict[str, Any]:
        with self._lock:
            try:
                ...
                self.logger.warning("[SESSES] Sessão %s no encontrada.", sessao_id[:8])
                return {}
            except Exception as e:
                self.logger.error("[SESSES] Erro ao obter dados sessão %s: %s", sessao_id[:8], e, exc_info=True)
                return {}
    ...
    def carregar_contexto_completo(self, sessao_id: str, limite_turnos: Optional[int] = None) -> str:
        with self._lock:
            try:
                ...
                self.logger.error("[SESSES] Erro ao carregar contexto %s: %s", sessao_id[:8], e, exc_info=True)
    ...
    def registrar_turno(self, sessao_id: str, input_usuario: str, resposta_ia: str) -> int:
        with self._lock:
            try:
                ...
                row = cursor.fetchone()
                if row is None:
                    self.logger.error("[SESSES] Sessão %s no encontrada para registrar turno.", sessao_id[:8])
                    raise GerenciadorSessoesError(f"Sessão '{sessao_id[:8]}' no encontrada.Turno no registrado.")
                ...
                self.conexão.commit()
                self.logger.debug("[SESSES] Turno %d registrado para sessão %s", proximo_turno, sessao_id[:8])
                return proximo_turno
            except Exception as e:
                self.logger.error("[SESSES] Erro ao registrar turno para sessão %s: %s", sessao_id[:8], e, exc_info=True)
                self.conexão.rollback()
    ...
    def arquivar_para_m3_com_resumo(self, sessao_id: str, resumo_texto: str) -> bool:
        with self._lock:
            try:
                ...
                row = cursor.fetchone()
                if not row:
                    self.logger.error("[SESSES] Sessão %s no encontrada para arquivamento M3.", sessao_id[:8])
                    return False
                resumivel, tema = row
                if not resumivel:
                    self.logger.warning("[SESSES] Sessão %s no marcada como resumvel; no ser arquivada.", sessao_id[:8])
                    return False
                cursor.execute(
                    """UPDATE sessoes
                ...
                self.conexão.commit()
                self.logger.info("[SESSES] Sessão %s arquivada em M3 com resumo.", sessao_id[:8])
                return True
            except Exception as e:
                self.logger.error("[SESSES] Erro ao arquivar sessão %s em M3: %s", sessao_id[:8], e, exc_info=True)
                self.conexão.rollback()
                return False
    ...
    def arquivar_para_m3_sem_resumo(self, sessao_id: str) -> bool:
        with self._lock:
            try:
                ...
                self.conexão.commit()
                self.logger.info("[SESSES] Sessão %s arquivada em M3 (sem resumo).", sessao_id[:8])
                return True
            except Exception as e:
                self.logger.error("[SESSES] Erro ao arquivar sessão %s sem resumo: %s", sessao_id[:8], e, exc_info=True)
                self.conexão.rollback()
                return False
    ...
    def recuperar_de_m3(self, sessao_id: str) -> Dict[str, Any]:
        with self._lock:
            try:
                ...
                row = cursor.fetchone()
                if not row:
                    self.logger.warning("[SESSES] Sessão %s no encontrada ação recuperar de M3.", sessao_id[:8])
                    return {'erro': 'Sessão no encontrada.'}
                ...
                self.logger.error("[SESSES] Erro ao recuperar sessão %s de M3: %s", sessao_id[:8], e, exc_info=True)
                return {'erro': str(e)}
    ...
    def listar_conversas_ativas(self, user_id: str, camada: Optional[str] = None) -> List[Dict[str, Any]]:
        with self._lock:
            try:
                ...
                self.logger.error("[SESSES] Erro ao listar conversas para %s: %s", user_id, e, exc_info=True)
                return []
    ...
    def listar_conversas_por_tema(self, user_id: str, tema: str) -> List[Dict[str, Any]]:
        with self._lock:
            try:
                ...
                self.logger.error("[SESSES] Erro ao listar conversas por tema %s para %s: %s", tema, user_id, e, exc_info=True)
                return []
    ...
    def exportar_conversa_texto(self, sessao_id: str) -> str:
        with self._lock:
            try:
                ...
                row = cursor.fetchone()
                if not row:
                    return f"Sessão '{sessao_id[:8]}' no encontrada."
                ...
                self.logger.error("[SESSES] Erro ao exportar sessão %s: %s", sessao_id[:8], e, exc_info=True)
    ...
    def contar_turnos(self, sessao_id: str) -> int:
        with self._lock:
            try:
                ...
                self.logger.error("[SESSES] Erro ao contar turnos para %s: %s", sessao_id[:8], e, exc_info=True)
                return 0
    ...
    def deletar_sessao(self, sessao_id: str) -> bool:
        with self._lock:
            try:
                ...
                self.conexão.commit()
                self.logger.info("[SESSES] Sessão %s deletada.", sessao_id[:8])
                return True
            except Exception as e:
                self.logger.error("[SESSES] Erro ao deletar sessão %s: %s", sessao_id[:8], e, exc_info=True)
                self.conexão.rollback()
                return False
    ...
    def transicionar_m1_para_m2(self) -> int:
        with self._lock:
            try:
                ...
                if total > 0:
                    self.logger.info("[SESSES] %d sessões movidas M1->M2.", total)
                return total
            except Exception as e:
                self.logger.error("[SESSES] Erro transicionando M1->M2: %s", e, exc_info=True)
                self.conexão.rollback()
                return 0
    ...
    def transicionar_m2_para_m3(self) -> int:
        with self._lock:
            try:
                ...
                total = cursor.rowcount
                if total > 0:
                    self.logger.info("[SESSES] %d sessões movidas M2->M3.", total)
                return total
            except Exception as e:
                self.logger.error("[SESSES] Erro transicionando M2->M3: %s", e, exc_info=True)
                self.conexão.rollback()
                return 0
    ...
    def obter_estatisticas(self) -> Dict[str, Any]:
        with self._lock:
            try:
                ...
                self.logger.error("[SESSES] Erro ao gerar estatsticas: %s", e, exc_info=True)
                return {}
    ...
    def desligar(self) -> None:
        with self._lock:
            try:
                if self.conexão:
                    self.conexão.close()
                self.logger.info("[SESSES] Gerenciador desligado.Conexo fechada.")
            except Exception as e:
                self.logger.error("[SESSES] Erro ao desligar: %s", e, exc_info=True)
