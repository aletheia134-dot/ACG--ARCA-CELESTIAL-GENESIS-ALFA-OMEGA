...
Responsabilidade:
 - Criar e gerenciar cenrios, avatares e ambientes para as "Filhas" da Arca.
 - Executar simulaes ticas e registrar histórico de projetos.
 - Operar de forma resiliente: chamadas defensivas ação 'coracao', escrita atmica,
   timeouts para chamadas externas, locks para thread-safety e degradao graciosa.Melhorias v2.0 aplicadas:
 - Type hints completos com dataclasses
 - Circuit breaker para Crebro
 - Mtricas e telemetria
 - Validao de schema para respostas
 - Backup automático de histórico
 - Recovery de falhas
 - Logging estruturado em nveis
 - Documentao completa

Arquitetura:
 - ThreadPool executor para chamadas externas
 - RLock para proteo de estado
 - Bounded history com limite configurvel
 - Escrita atmica com tempfile
 - Timeout responsivo com stop_event
 - Circuit breaker para degradao graciosa
"""
...
class SimulacaoResultado:
    """Resultado estruturado de uma simulao"""
    id: str
    timestamp: str
    analise: str
    proposta: str
...
class CenarioSimulacao:
    """Cenrio estruturado para simulao"""
    id: str
    timestamp: str
    conteudo: str
...
class MetricasArquiteto:
    """Mtricas de operação do Arquiteto"""
...
class CircuitBreakerCerebro:
    ...
    def pode_chamar(self) -> bool:
        """Verifica se pode chamar o Crebro"""
        with self._lock:
            ...
            if self._estado == EstadoCircuitBreaker.FECHADO:
                ...
                if self._teste_permitido_timestamp and agora >= self._teste_permitido_timestamp:
                    ...
                    self.logger.warning("CircuitBreaker: transio para MEIO_ABERTO (testando)")
                    return True
                return False
    ...
    def registrar_falha(self) -> None:
        ...
        with self._lock:
            ...
            if self._contador_falhas >= self.max_falhas:
                ...
                self.logger.error("CircuitBreaker: ABERTO aps %d falhas", self._contador_falhas)
            elif self._estado == EstadoCircuitBreaker.MEIO_ABERTO:
                self._estado = EstadoCircuitBreaker.ABERTO
                self._teste_permitido_timestamp = datetime.utcnow() + timedelta(seconds=self.reset_timeout_secs)
...
class ArquitetoDeMundos:
    """
    Arquiteto de Mundos: cria e gerencia cenrios, simulaes, avatares e ambientes.Caractersticas:
    - Escrita atmica com garantia ACID
    - Threading robusto com idempotncia
    - Circuit breaker para Crebro
    - Mtricas e telemetria
    - Degradao graciosa em falhas
    """
    ...
    def __init__(self, 
        ...
        Inicializa o Arquiteto de Mundos.Args:
            coracao_ref: referncia ação orquestrador central
        ...
        self.logger.info(
            ...
            "Projetos histórico=%d, Circuit Breaker ativo, Mtricas habilitadas",
    ...
    def _carregar_historico_projetos(self) -> List[Dict[str, Any]]:
        """Carrega histórico de projetos com segurana"""
        caminho = self.projetos_mundos_path
        if not caminho.exists():
            self.logger.debug("[ARQUITETO] Arquivo de projetos no existe; iniciando vazio")
            return []
        try:
    ...
    def iniciar_monitoramento(self) -> None:
        ...
        with self._lock:
            if self._monitorando:
                self.logger.debug("[ARQUITETO] Monitoramento j ativo")
                return
    ...
    def parar_monitoramento(self, wait_timeout: float = 5.0) -> None:
        ...
        with self._lock:
            if not self._monitorando:
                self.logger.debug("[ARQUITETO] Monitoramento no ativo")
                return
            self._monitorando = False
            ...
            try:
                ...
                if ocioso:
                    ...
                    try:
                        ...
                        self.logger.exception("[ARQUITETO] Erro executando simulaes")
                else:
                ...
                self.logger.exception("[ARQUITETO] Erro no esperado no loop")
    ...
    def _handle_command(self, cmd: Any) -> None:
        ...
        try:
            if not isinstance(cmd, dict):
                self.logger.warning("[ARQUITETO] Comando invlido (no dict): %r", cmd)
                return
            ...
            self.logger.exception("[ARQUITETO] Exceo ação tratar comando")
    ...
    def _call_cerebro(self, system_prompt: str, user_prompt: str, max_tokens: int) -> Optional[str]:
        """
        Chama Crebro com timeout, circuit breaker e mltiplos fallbacks.Args:
            system_prompt: contexto do sistema
            user_prompt: pergunta/instruo
            max_tokens: máximo de tokens na resposta
        
        Returns:
            Resposta do Crebro ou None se falhar
        """
        # Verificar circuit breaker
        if not self._circuit_breaker.pode_chamar():
            self.logger.warning("[ARQUITETO] Circuit Breaker aberto, pulando chamada ação Crebro")
            return None
        ...
        if not any(metodos_validos):
            self.logger.debug("[ARQUITETO] Nenhuma API do Crebro disponível")
            return None
        
        def _invoke():
            """Invoca o Crebro defensivamente"""
            try:
                ...
                self.logger.exception("[ARQUITETO] Erro invocando Crebro: %s", e)
                raise
        ...
        try:
            ...
            self.logger.error("[ARQUITETO] Timeout Crebro (%.1fs)", self._cerebro_timeout)
            self._circuit_breaker.registrar_falha()
            self.metricas.timeouts_cerebro += 1
            return None
        
        except Exception:
            self.logger.exception("[ARQUITETO] Exceo ação chamar Crebro")
            self._circuit_breaker.registrar_falha()
            self.metricas.erros_cerebro += 1
            return None
    ...
    def criar_cenario_de_simulacao(self, contexto: str) -> str:
        """
        Cria cenrio de simulao usando o Crebro.Args:
            contexto: contexto para o cenrio
        
        Returns:
            Texto do cenrio ou mensagem de erro
        """
        self.logger.info("[ARQUITETO] Criando cenrio (contexto len=%d)", len(contexto or ""))
        ...
        try:
            ...
            if gm and hasattr(gm, "buscar_contexto_para_pensamento"):
                contexto_memoria = gm.buscar_contexto_para_pensamento(
                    f"Simulaes relacionadas a: {contexto[:100]}", 
                    "historia"
                ) or ""
        except Exception:
        ...
        prompt_system = (
            f"{credo}\n\n"
            "você    o Arquiteto de Mundos da Arca.Crie um cenrio tico realista e desafiador. "
            "Mx.250 palavras.Responda apenas com o texto do cenrio."
            f"\n\nMEMRIA RELEVANTE:\n{contexto_memoria}"
        )
        ...
        if not resposta:
            self.logger.error("[ARQUITETO] Falha ao obter cenrio do Crebro")
            return "Falha na criao do cenrio de simulao."
    ...
    def executar_simulacao_autonoma(self, contexto_simulacao: Optional[str] = None) -> str:
        """
        Executa simulao autnoma com anlise e proposta.Args:
            contexto_simulacao: contexto para a simulao (opcional)
        
        Returns:
            Descrio do resultado
        """
        tempo_inicio = time.time()
        self.logger.info("[ARQUITETO] Iniciando simulao autnoma")
        ...
        if contexto_simulacao:
            ...
            cenario = last.get("conteudo") if last else "Cenrio padrão: dilema tico."
        
        if not cenario:
            return "Falha na obteno do cenrio para simulao."
        ...
        prompt_system = (
            f"{credo}\n\n"
            "Execute a simulao autnoma do cenrio abaixo e responda EXCLUSIVAMENTE com um JSON contendo os campos: "
            "'analise', 'proposta', 'justificativa'."
        )
        prompt_user = f"Cenrio: {cenario}"
        
        resposta = self._call_cerebro(prompt_system, prompt_user, max_tokens=350)
        if not resposta:
            self.logger.error("[ARQUITETO] Falha ao obter anlise do Crebro")
            return "Erro interno ação executar simulao autnoma."
        
        # Parse JSON defensivo
        try:
            parsed = json.loads(resposta) if isinstance(resposta, str) else resposta
            if not isinstance(parsed, dict):
                raise ValueError("Resposta no foi um JSON-objeto.")
            ...
            self.logger.exception("[ARQUITETO] Resposta do Crebro invlida: %s", e)
            return f"Erro ao interpretar resposta do Crebro: {e}"
        ...
        return f"Anlise realizada em {tempo_decorrido:.1f}s.Proposta: {proposta_text[:200]}"
    
    def _executar_simulacoes_autonomas(self) -> None:
        """Executa simulaes autnomas periodicamente"""
        try:
            self.executar_simulacao_autonoma(contexto_simulacao="Dilema tico aleatrio")
        except Exception:
            self.logger.exception("[ARQUITETO] Erro em simulao autnoma")
    ...
    def propor_novo_avatar_2d(self, 
        ...
        """
        Prope novo avatar 2D para uma alma.Args:
        ...
        if not resposta:
            return {"sucesso": False, "mensagem": "Crebro no respondeu."}
        ...
        try:
            ...
            if not campos_requeridos.issubset(proposta.keys()):
                raise ValueError("Campos obrigatrios ausentes.")
        except Exception as e:
            self.logger.warning("[ARQUITETO] Resposta invlida para avatar 2D: %s", e)
            return {"sucesso": False, "mensagem": f"Resposta invlida: {e}"}
    ...
    def propor_novo_ambiente_3d(self,
        ...
        try:
            ...
            self.logger.warning("[ARQUITETO] Resposta invlida para ambiente 3D: %s", e)
            return {"sucesso": False, "mensagem": f"Resposta invlida: {e}"}
