...
Gerencia carregamento e interao com LLMs locais (via llama_cpp) e um servio RAG.Projetado para degradar graciosamente quando dependncias no esto presentes
(e.g., em ambientes de desenvolvimento/test) e para aplicar timeouts nas chamadas
de inferncia.Principais endurecimentos:
 - Imports defensivos (llama_cpp opcional)
 - Injeo de dependncia para RAGService (ou tentativa de import fallback)
 - Verificaes de existncia de arquivos de modelo
 - execução de inferncia com ThreadPoolExecutor + timeout configurvel
 - Logging em vez de prints
 - Estado consultvel (is_phi3_loaded, is_vikhr_loaded)
 - Métodos de shutdown para liberar recursos do executor
 - Parmetros configurveis via construtor
"""
...
try:
    ...
    logging.getLogger(__name__).warning("[AVISO] Llama no disponível")
    Llama = None  # type: ignore
    _LLAMA_AVAILABLE = False
    logger.warning("llama_cpp no disponível; LLMService entrar em modo degradado (modelos locais desativados).")
...
try:
    ...
    logger.info("RAGService no encontrado via import; aceitar injeo via construtor se necessário.")


class LLMService:
    """
    Servio de LLM + RAG endurecido.Args:
        model_dir: diretório base onde ficam os modelos.phi3_model_path / vikhr_model_path: caminhos relativos ou absolutos para os modelos GGUF.rag_service: instncia existente de RAGService (opcional). Se no fornecido, tentamos importar.n_gpu_layers: número de camadas na GPU (pass-through para llama_cpp).
        inference_timeout: tempo máximo (segundos) para cada chamada de inferncia.executor_workers: número de workers para executar inferncias em threads.
    """
    ...
    def _load_model_instance(self, model_path: Path) -> Optional[Any]:
        ...
        if not _LLAMA_AVAILABLE or Llama is None:
            logger.debug("llama_cpp no disponível; no  possível carregar: %s", model_path)
            return None
        if not self._model_file_ok(model_path):
            logger.warning("Arquivo de modelo no encontrado: %s", model_path)
            return None
        try:
            llm = Llama(
    ...
    def initialize(self, load_phi3: bool = True, load_vikhr: bool = True) -> None:
        """
        Carrega modelos solicitados e inicializa RAG se necessário.Esta função  segura para chamadas repetidas (far reload se j houver instncias).
        """
        if self._shutdown:
    ...
    def _invoke_llm_sync(self, llm_instance: Any, messages: list, max_tokens: int, temperature: float):
        """
        Invoca a API do LLM de forma defensiva.Retorna string de contedo ou eleva Exception.Suporta create_chat_completion e create_completion (fallback).
        """
        ...
        try:
            ...
            logger.exception("Erro na chamada sncrona do LLM.")
            raise
    ...
    def _run_inference_with_timeout(self, llm_instance: Any, messages: list, max_tokens: int, temperature: float, timeout: Optional[float] = None) -> str:
        """
        Executa a inferncia em thread separada com timeout.Se a chamada exceder o timeout, retorna mensagem de erro e cancela (no mata thread).
        """
        if timeout is None:
        ...
        try:
            ...
            logger.error("Inferncia ultrapassou timeout de %.1f segundos.", timeout)
            return f"Erro: inferncia excedeu o timeout de {timeout} segundos."
        except Exception as e:
            logger.exception("Erro durante inferncia: %s", e)
            return f"Erro na inferncia: {e}"

    # -------------------------
    # High-level utilities
    # -------------------------
    def _build_messages(self, system_prompt: str, user_prompt: str):
        """Constroi lista de mensagens em formato compatvel com chat APIs."""
        return [
    ...
    def _safe_query_rag(self, user_input: str) -> str:
        ...
        if not self.rag_service:
            logger.debug("RAGService no configurado; retornando contexto vazio.")
            return ""
        try:
    ...
    def process_request(self, user_input: str, mode: str = "phi3") -> Dict[str, str]:
        """
        Processa uma requisio do usurio.mode in {'phi3','vikhr','collective'}
        Retorna dict com chaves de modelos e seus textos.
        """
        if self._shutdown:
            return {"error": "LLMService est desligado."}
    ...
    def shutdown(self, wait: bool = True, timeout: float = 5.0) -> None:
        """Encerramento limpo do servio (executor)."""
        self._shutdown = True
        try:
