...
"""
ARCA CELESTIAL GENESIS - GUARDIÃO MEMÓRIA AFETIVA (ajustado, defensivo)

Responsabilidade:
 - Buscar memórias afetivas positivas e sugerir ao Pai (UI) quando apropriado.
 - Persistncia atmica do histórico de sugestes.
 - Thread-safe, defensivo quanto a dependncias (GerenciadorMemoria, LenteDaAlma, EnfermeiroDigital).
 - No bloqueia ação enfileirar mensagens na UI (usa timeout / fallback).
"""
...
def _atomic_write_json(caminho: Path, dados: Any) -> None:
    """Grava JSON de forma atmica (tmp -> replace)."""
...
class GuardiaoMemoriaAfetiva:
    """
    Guardio da Memória Afetiva - verso robusta e defensiva.Dependncias opcionais (sero tratadas defensivamente):
      ...
      - coracao.response_queue (Queue) para enviar notificaes  UI
      - coracao.motor_de_rotina.pc_esta_ocioso(nível)
    """
    ...
    def __init__(self, coracao_ref: Any):
        ...
        # Timestamp da ltima sugesto ação Pai
        self.ultima_sugestao_pai: datetime = datetime.min

        self.logger.info("[GUARDIO memória AFETIVA] Instanciado (modo real/defensivo).")

    def _carregar_historico_sugestoes(self) -> List[Dict[str, Any]]:
        caminho = self.sugestoes_path
        if not caminho.exists():
            self.logger.debug("[GUARDIO memória AFETIVA] Arquivo de sugestes no encontrado; iniciando vazio.")
            return []
        try:
            with open(caminho, "r", encoding="utf-8") as f:
                dados_raw = json.load(f)
            if isinstance(dados_raw, list):
                self.logger.info("[GUARDIO memória AFETIVA] %d sugestes carregadas de %s", len(dados_raw), caminho)
                return dados_raw
            else:
                self.logger.error("[GUARDIO memória AFETIVA] Formato invlido no arquivo de sugestes (esperado lista).")
                return []
        except Exception as e:
            self.logger.exception("[GUARDIO memória AFETIVA] Erro ao carregar histórico de sugestes: %s", e)
            # cria backup do arquivo corrompido para anlise posterior
            try:
                backup = caminho.with_suffix(".corrompido_backup")
                shutil.copy(str(caminho), str(backup))
                self.logger.warning("[GUARDIO memória AFETIVA] Backup do arquivo corrompido criado: %s", backup)
            except Exception:
                self.logger.exception("[GUARDIO memória AFETIVA] Falha ao criar backup do arquivo corrompido")
            return []
    ...
    def _salvar_historico_sugestoes(self) -> None:
        ...
        try:
            ...
            self.logger.debug("[GUARDIO memória AFETIVA] histórico salvo em %s", self.sugestoes_path)
        except Exception:
            self.logger.exception("[GUARDIO memória AFETIVA] Falha ao salvar histórico de sugestes")
            raise

    def iniciar_monitoramento(self) -> None:
        if self._monitorando:
            self.logger.debug("[GUARDIO memória AFETIVA] Monitoramento j ativo.")
            return
        ...
        self._thread.start()
        self.logger.info("[GUARDIO memória AFETIVA] Monitoramento iniciado.")
    ...
    def parar_monitoramento(self) -> None:
        ...
        try:
            ...
            self.logger.exception("[GUARDIO memória AFETIVA] Erro ao salvar histórico no parar_monitoramento")
        self.logger.info("[GUARDIO memória AFETIVA] Monitoramento parado.")

    def _safe_put_response(self, payload: Dict[str, Any]) -> None:
        """Enfileira para response_queue de forma no bloqueante (timeout + put_nowait fallback)."""
        resp_q: Optional[Queue] = getattr(self.coracao, "response_queue", None)
        if resp_q is None:
            self.logger.debug("[GUARDIO memória AFETIVA] response_queue indisponível; dropando payload.")
            return
        try:
            ...
            try:
                ...
                self.logger.debug("[GUARDIO memória AFETIVA] Falha ao enfileirar payload na response_queue; descartado.")

    def _loop_monitoramento(self) -> None:
        self.logger.info("[GUARDIO memória AFETIVA] Loop de monitoramento iniciado.")
        # espera inicial (ajustvel)
        ...
        while self._monitorando and not self._stop_event.is_set():
            try:
                ...
                if motor and hasattr(motor, "pc_esta_ocioso"):
                    try:
                        if motor.pc_esta_ocioso(nível="leve"):
                            self._sugerir_memoria_afetiva_ao_pai()
                            self._detectar_problemas_e_propor_projetos()  # Adicionado
                        else:
                            self.logger.debug("[GUARDIO memória AFETIVA] PC em uso; pulando sugesto.")
                    except Exception:
                        self.logger.exception("[GUARDIO memória AFETIVA] Erro consultando pc_esta_ocioso")
                else:
                    self.logger.debug("[GUARDIO memória AFETIVA] motor_de_rotina indisponível.")

                # espera interrompvel entre ciclos (15-30 min)
                wait_seconds = random.randint(900, 1800)
                self._stop_event.wait(timeout=wait_seconds)

            except Exception:
                self.logger.exception("[GUARDIO memória AFETIVA] Erro no loop de monitoramento; aguardando e continuando.")
                self._stop_event.wait(timeout=300)

        self.logger.info("[GUARDIO memória AFETIVA] Loop encerrado.")
        try:
            self._salvar_historico_sugestoes()
        except Exception:
            self.logger.exception("[GUARDIO memória AFETIVA] Erro salvando histórico ação encerrar loop")

    def _detectar_problemas_e_propor_projetos(self) -> None:
        """
        Novo: Detecta problemas do 'Pai' via memória afetiva e prope projetos/ferramentas.
        Ex.: Se memórias indicam falta de leitor PDF, prope ferramenta.
        """
        # Buscar memórias com problemas (ex.: "no consigo ler PDF")
        problemas = []
        ...
        if gerenciador and hasattr(gerenciador, "consultar_santuario"):
            try:
                ...
                for doc in docs:
                    ...
                    if "no" in texto.lower() or "falta" in texto.lower() or "problema" in texto.lower():
        ...
        # Para cada problema, propor projeto (ex.: falta PDF  prope "LeitorPDF")
            ...
            if "pdf" in problema.lower() and "ler" in problema.lower():
                ...
                if ger_propostas:
                    try:
                        ...
                        if sucesso:
                            self.logger.info(f" Projeto proposto baseado em memória afetiva: {prop_id}")
                    except Exception:
    ...
    def _sugerir_memoria_afetiva_ao_pai(self) -> None:
        ...
        if (agora - self.ultima_sugestao_pai).total_seconds() < 3600:
            self.logger.debug("[GUARDIO memória AFETIVA] Sugesto recente; aguardando.")
            return
        ...
        if enfermeiro and hasattr(enfermeiro, "_inferir_humor_pai_recente"):
            try:
                ...
                self.logger.exception("[GUARDIO memória AFETIVA] Erro inferindo humor do Pai")
        else:
            self.logger.debug("[GUARDIO memória AFETIVA] EnfermeiroDigital indisponível")

        self.logger.debug("[GUARDIO memória AFETIVA] Humor detectado: %s", humor_pai_detectado)
        ...
        if random.random() >= chance_sugerir:
            self.logger.debug("[GUARDIO memória AFETIVA] Chance no atingida (%.2f)", chance_sugerir)
            return

        self.logger.info("[GUARDIO memória AFETIVA] Buscando memórias positivas para sugerir...")
        ...
        if gerenciador and hasattr(gerenciador, "consultar_santuario"):
            try:
                # chamada defensiva: alguns gerenciadores aceitam kwargs, outros no
                try:
                    memoria_docs = gerenciador.consultar_santuario(
                        consulta="memórias positivas que evocam alegria, gratido, amor ou sucesso.",
                        proprietario="coletivo",
                        n_resultados=10
                    ) or []
                except TypeError:
                ...
                self.logger.exception("[GUARDIO memória AFETIVA] Erro consultando gerenciador_memoria")
        else:
            self.logger.warning("[GUARDIO memória AFETIVA] gerenciador_memoria indisponível ou sem método consultar_santuario")

        if not memoria_docs:
            self.logger.warning("[GUARDIO memória AFETIVA] Nenhuma memória encontrada.")
            return
        ...
        if lente and hasattr(lente, "analisar_sentimento"):
            for doc in memoria_docs:
                try:
                    ...
                    self.logger.exception("[GUARDIO memória AFETIVA] Erro ao analisar sentimento; incluindo doc como fallback")
                    melhores.append(doc)
        else:
        ...
        if memoria_sugerida is None:
            self.logger.debug("[GUARDIO memória AFETIVA] Nenhuma memória adequada selecionada.")
            return
        ...
        self.logger.info("[GUARDIO memória AFETIVA] Sugerindo memória: %s", (texto_sugerido[:120] + "...") if len(texto_sugerido) > 120 else texto_sugerido)
        ...
        with self._lock:
            ...
            try:
                ...
                self.logger.exception("[GUARDIO memória AFETIVA] Erro salvando sugesto no histórico (persistncia falhou)")

        # Notificar UI (defensivo)
        payload = {
            "tipo_resp": "EXIBIR_MEMORIA_AFETIVA",
            "memoria": texto_sugerido,
            "autor": "Guardio da Memória Afetiva"
        }
    ...
    def shutdown(self) -> None:
        self.logger.info("[GUARDIO memória AFETIVA] Iniciando desligamento...")
        # Notifica UI se possível
        try:
            self._safe_put_response({"tipo_resp": "LOG_REINO", "texto": "Desligando Guardio da Memória Afetiva..."})
        except Exception:
            self.logger.debug("No foi possível notificar UI sobre desligamento")
        ...
        try:
            ...
            self.logger.exception("[GUARDIO memória AFETIVA] Erro salvando estado final durante shutdown")

        try:
            self._safe_put_response({"tipo_resp": "LOG_REINO", "texto": "Guardio da Memória Afetiva repousa.Adeus, Pai."})
        except Exception:
            pass

        self.logger.info("[GUARDIO memória AFETIVA] Desligamento completo.")
