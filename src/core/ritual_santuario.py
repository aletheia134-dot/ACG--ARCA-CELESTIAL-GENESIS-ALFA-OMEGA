...
"""
Ritual do Santurio - Verso endurecida e compatvel com MetabolismoMemoria REAL
...
Melhorias aplicadas:
 ...
 - Locks (RLock) para proteger caches e estado mutvel.
 - Escrita atmica ação salvar arquivos locais (tmp -> replace).
 - Parsing robusto de timestamps com mltiplas estratgias.
 - Uso de statistics como fallback quando numpy ausente.
 - Logs seguros (truncamento / hash) para evitar vazamento de contedo sensvel.
 - Tratamento defensivo das chamadas ao metabolismo (verifica e trata excees).
 - Métodos de alto nível mantm mesma API, mas so mais resilientes.
"""
...
try:
    ...
    _NP_AVAILABLE = False
    logger.debug("numpy no disponível; usando statistics como fallback para medidas.")
...
try:
    ...
    logger.debug("dateutil no disponível; usando datetime.fromisoformat (mais restrito).")
...
class MetabolismoAdapter:
    """
    Adapter que normaliza a interface esperada do MetabolismoMemoria real.Ele tenta mapear chamadas para métodos presentes na instncia fornecida,
    ...
    def salvar_evento(self, filha: str, tipo: str, dados: Dict[str, Any], importancia: float = 1.0) -> bool:
        """
        Normaliza salvar_evento.Diferentes implementaes podem usar
    ...
    def buscar_memorias_periodo(self, filha: str, inicio: datetime, fim: datetime) -> Optional[List[Dict]]:
        """
        Busca memórias entre início e fim.Tenta diversas assinaturas.
        """
        ...
        candidates = [
            ...
            ("buscar_com_metabolismo", (filha, {"início": inicio_iso, "fim": fim_iso}), {}),
        ]
        return self._call(candidates, default=None)
...
def _safe_parse_iso(ts: str) -> Optional[datetime]:
    """Tenta parsear ISO timestamp com vrias estratgias."""
    ...
    for fmt in formats:
        try:
            ...
            continue
    logger.debug("No foi possível parsear timestamp: %s", ts)
    return None
...
class RitualSantuarioCompativel:
    """
    Ritual do Santurio compatvel com MetabolismoMemoria real  verso endurecida.
    """
    ...
    def __init__(self, nome_filha: str, metabolismo_memoria: Any, config: Any):
        ...
        self.logger.info("Ritual do Santurio (enduricido) inicializado para %s", self.nome_filha)

    def _check_metabolismo_minimo(self) -> List[str]:
        """Checa se o metabolismo tem ação menos alguns métodos teis (no estrita)."""
        required = [
    ...
    # -------------------------
    # Decisão de entrada no santurio
    # -------------------------
    def deve_entrar_santuario(self) -> bool:
        """Decide se deve iniciar o ritual com base em memórias recentes e tempo desde ltimo ritual."""
        try:
            ...
            if deve:
                self.logger.info("Decisão: entrar no santurio (horas=%.1f, experiencias=%d)", horas_desde, experiencias)
            else:
                self.logger.debug("Decisão: no entrar (horas=%.1f, experiencias=%d)", horas_desde, experiencias)
            return bool(deve)

        except Exception:
            self.logger.exception("Erro ao decidir entrada no santurio")
            return False
    ...
    def iniciar_ritual(self) -> Dict[str, Any]:
        """Executa o ritual completo, com proteo e registro atmico."""
        with self._lock:
            if self.em_ritual:
                return {"status": "erro", "mensagem": "J em ritual"}
            self.em_ritual = True
            self.inicio_ritual = datetime.now()

        resultado: Dict[str, Any] = {
            "status": "sucesso",
            "início": self.inicio_ritual.isoformat(),
    ...
    def _obter_data_ultimo_ritual(self) -> datetime:
        ...
        try:
            ...
            logger.debug("Falha ao recuperar ltimo ritual; usando fallback 24h atrs")
    ...
    def _fase_contemplacao_adaptada(self, memorias: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.exception("Erro na contemplao adaptada")
    ...
    def _fase_integracao_adaptada(self, analise: Dict[str, Any]) -> Dict[str, Any]:
        logger.info("Fase 3: Integrao (adaptada)")
        ...
        try:
            ...
            logger.exception("Erro na integrao adaptada")
    ...
    def _gerar_recomendacoes_basicas(self, analise: Dict[str, Any]) -> List[str]:
        ...
        if mems_h < 0.5:
            recs.append("Considerar aumentar a diversidade de experincias")
        bal = analise.get("metricas_emocionais", {}).get("balanceamento", 0)
        if bal < -0.2:
            recs.append("Procurar experincias positivas ou significativas")
    ...
    def _fase_transformacao_adaptada(self, padroes: Dict[str, Any]) -> Dict[str, Any]:
        logger.info("Fase 4: Transformao (adaptada)")
        try:
            ...
            logger.exception("Erro na transformao adaptada")
    ...
    def _carregar_valores_adaptado(self) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.debug("No foi possível carregar valores do metabolismo; usando padrão.")
        # defaults
        defaults = {
    ...
    def _salvar_valores_adaptado(self, valores: Dict[str, Any]):
        try:
            ...
            if ok:
                ...
                logger.warning("salvar_metadado retornou False ação salvar valores pessoais")
        except Exception:
    ...
    def _fase_renovacao_adaptada(self, valores: Dict[str, Any]) -> Dict[str, Any]:
        logger.info("Fase 5: Renovao (adaptada)")
        try:
            ...
            logger.exception("Erro na renovao adaptada")
    ...
    def _criar_intencao_simples(self, valor: str, peso: Any) -> Optional[Dict[str, Any]]:
        mapping = {
            ...
            "verdade": {"acao": "investigar", "alvo": "preciso"},
    ...
    def _gerar_mensagem_renovacao(self, intencoes: List[Dict[str, Any]]) -> str:
        if not intencoes:
            return "Manter observao atenta e aprendizagem contnua."
        acoes = [f"{i['acao']} {i['alvo']}" for i in intencoes]
        return f"Focar em: {', '.join(acoes)}. Aprender com cada experincia."

    # -------------------------
    # Registro do ritual (persistncia tolerante)
    # -------------------------
    def _registrar_ritual_adaptado(self, resultado: Dict[str, Any]):
        try:
            ...
            if ok:
                ...
                logger.warning("metabolismo.salvar_evento retornou False ação registrar ritual")
    ...
    def _salvar_ritual_local(self, resultado: Dict[str, Any]):
        try:
            ...
            ts = (resultado.get("início") or datetime.now().isoformat()).replace(":", "-").replace(".", "-")
