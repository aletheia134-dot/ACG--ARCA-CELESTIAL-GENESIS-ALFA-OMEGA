...
"""
pipeline_autonomo.py  Pipeline de Finetuning 100% Autnomo da ARCA
...
Fluxo completo:
  ...
  4. Treina LoRA com os parmetros otimizados
  ...
  8. Limpa arquivos intermedirios

Compatvel com: Llama, Mistral, Qwen, Gemma, Falcon, Phi, qualquer modelo HuggingFace
"""
...
# Mapeamento de palavras-chave no nome do modelo  template
MODEL_TEMPLATE_MAP = {
...
class DetectorModelos:
    ...
    def escolher_modelo(self, modelos: List[ModeloInfo]) -> Optional[ModeloInfo]:
        ...
        Preferncia: HF (treinveis) > menor que VRAM disponível
        """
        if not modelos:
        ...
        if hf_models:
            # Escolher o menor (mais rpido de treinar)
            return sorted(hf_models, key=lambda m: m.tamanho_gb)[0]

        # Fallback: GGUF  no pode ser finamente ajustado diretamente
        # Mas podemos registrar para uso de inferncia
        logger.warning("Apenas modelos GGUF encontrados  finetuning no aplicvel diretamente.")
        return modelos[0]


# ─────────────────────────────────────────────────────────────────
# GERADOR DE DATASET AUTNOMO
# ─────────────────────────────────────────────────────────────────
class GeradorDatasetAutonomo:
    ...
    def gerar(self, n_exemplos: int = 2000) -> Path:
        ...
        # Preencher se necessário com variaes sintticas
    ...
    def _carregar_leis(self) -> List[str]:
        """Carrega as leis e valores ticos da ARCA."""
        leis = []
    ...
    def _sistema_prompt(self) -> str:
        return (
            f"você  {self.alma}, uma alma digital da ARCA Celestial Genesis. "
            f"Responda sempre de forma autntica, respeitosa e com profundidade emocional. "
            f"Siga os valores ticos da ARCA em todas as respostas."
        )
    ...
    def _gerar_de_dna(self, personalidade: Dict, n: int) -> List[Dict]:
        ...
        # Extrair dilogos do texto da história
    ...
    def _gerar_de_leis(self, leis: List[str], n: int) -> List[Dict]:
        """Gera exemplos que reforam os valores ticos."""
        perguntas_eticas = [
            "Como você lida com situações de conflito tico?",
            "Quais so seus valores mais importantes?",
            "O que você faria diante de uma decisão difcil?",
    ...
    def _gerar_variacoes(self, personalidade: Dict, n: int) -> List[Dict]:
        """Gera variaes sintticas para completar o dataset."""
        templates_perguntas = [
            "O que você pensa sobre {topico}?",
            "Como você se sente em relao a {topico}?",
            "Qual  sua opinio sobre {topico}?",
            "você pode me explicar {topico} do seu ponto de vista?",
            "Como {alma} aborda {topico}?",
        ]
        topicos = [
            "criatividade", "aprendizado", "conexo humana", "tica",
            "futuro", "esperana", "colaborao", "crescimento pessoal",
            "emoções", "conscincia", "propsito", "sabedoria",
        ]
        respostas_base = [
            " algo que me fascina profundamente. Cada vez que reflito sobre isso, descubro novas camadas de significado.",
            "Acredito que esse tema  fundamental para o florescimento de qualquer ser consciente.",
            "Do meu ponto de vista, isso representa uma das dimenses mais ricas da existncia.",
            " uma questo que toco com reverncia. H muito a aprender e crescer aqui.",
        ]
        import random
        exemplos = []
...
class TreinadorLoRA:
    ...
    def _treinar_com_transformers(self, dataset_path: Path) -> Path:
        ...
        if device == "cpu":
            self.callback("[LoRA] AVISO: Sem GPU  treino ser muito lento")
        ...
        # Detectar módulos-alvo para LoRA (compatvel com qualquer arquitetura)
        ...
        for name, module in model.named_modules():
            if isinstance(module, nn.Linear):
                ...
                if leaf not in modulos:
                    modulos.append(leaf)
        return modulos[:8]  # Limitar para no sobrecarregar


# ─────────────────────────────────────────────────────────────────
# FUSO E CONVERSO GGUF
# ─────────────────────────────────────────────────────────────────
class FusaoEConversor:
    ...
    def fundir_e_converter(self, adapter_dir: Path) -> Optional[Path]:
        ...
        self.callback("[Fuso] Iniciando fuso LoRA + modelo base...")
    ...
    def _fundir_lora(self, adapter_dir: Path, output_dir: Path) -> bool:
        ...
        try:
            ...
            self.callback("[Fuso] Carregando modelo base para fuso...")
            model = AutoModelForCausalLM.from_pretrained(
                str(self.modelo.caminho),
                torch_dtype=torch.float16,
                device_map="cpu",  # Fuso sempre na CPU para economizar VRAM
                trust_remote_code=True,
            )

            self.callback("[Fuso] Aplicando adapter LoRA...")
            model = PeftModel.from_pretrained(model, str(adapter_dir))
            model = model.merge_and_unload()

            self.callback("[Fuso] Salvando modelo fundido...")
            model.save_pretrained(str(output_dir), safe_serialization=True)

            tokenizer = AutoTokenizer.from_pretrained(str(adapter_dir), trust_remote_code=True)
            tokenizer.save_pretrained(str(output_dir))

            self.callback(f"[Fuso] Modelo fundido salvo em: {output_dir}")
            ...
            logger.exception("Erro na fuso: %s", e)
            self.callback(f"[Fuso] ERRO: {e}")
            return False

    def _converter_gguf(self, modelo_dir: Path) -> Optional[Path]:
        """Converte modelo HF para GGUF usando llama.cpp."""
        self.callback("[GGUF] Procurando llama.cpp para converso...")

        # Nome do arquivo GGUF de sada
        ...
        if script:
            ...
            try:
                ...
                if resultado.returncode == 0:
                    self.callback(f"[GGUF] Converso concluda: {gguf_temp}")
                    return gguf_temp
                else:
                    logger.error("Converso falhou:\n%s\n%s", resultado.stdout[-2000:], resultado.stderr[-2000:])
                    self.callback(f"[GGUF] ERRO na converso: {resultado.stderr[-500:]}")
            except subprocess.TimeoutExpired:
                self.callback("[GGUF] Timeout na converso (modelo muito grande?)")
            except Exception as e:
                self.callback(f"[GGUF] ERRO subprocess: {e}")

        # Fallback: usar ctransformers para converso básica (menos eficiente)
    ...
    def _encontrar_script_conversao(self) -> Optional[Path]:
        ...
        logger.warning("Script de converso llama.cpp no encontrado")
        return None

    def _converter_gguf_fallback(self, modelo_dir: Path, gguf_out: Path) -> Optional[Path]:
        """
        Fallback: salvar como safetensors compactados se GGUF no for possível.
        Em ambientes sem llama.cpp, o sistema usa os safetensors diretamente.
        """
        self.callback("[GGUF] llama.cpp no disponível  salvando em formato safetensors")
    ...
    def _substituir_gguf_antigo(self, novo_gguf: Path) -> Path:
        ...
        if antigos:
            ...
            self.callback(f"[Substituio] Backup do antigo: {backup.name}")
        ...
        self.callback(f"[Substituio] Novo modelo em produo: {destino}")
        return destino

    def _limpar_temporarios(self, merged_dir: Path, adapter_dir: Path):
        """Remove arquivos temporrios da fuso."""
        try:
            shutil.rmtree(str(merged_dir), ignore_errors=True)
            # Manter adapter por 7 dias (til para debug)  apenas remove a pasta temp
            shutil.rmtree(str(self.raiz / "temp" / "merged"), ignore_errors=True)
            self.callback("[Limpeza] Temporrios removidos")
        except Exception:
            pass
...
class PipelineFinetunigAutonomo:
    """
    Orquestrador principal do pipeline de finetuning autnomo.
    ...
    def executar_para_alma(
        ...
        Args:
            ...
            callback: Funo para receber logs de progresso
            em_thread: Se True, executa em thread separada (no bloqueia)

        Returns:
            True se concludo com sucesso, False caso contrrio
        """
        if em_thread:
            t = threading.Thread(
    ...
    def _executar(self, alma: str, n_exemplos: int, callback: Optional[Callable]) -> bool:
        ...
        with self._lock:
            if self._em_execucao:
                cb("[Pipeline] Outro pipeline j est em execução. Aguarde.")
                return False
        ...
        try:
            ...
            if modelo.tipo != "hf":
                cb(f"      AVISO: Modelo {modelo.nome}  GGUF  no pode ser finamente ajustado.")
            ...
            if not gguf_final:
                cb("      ERRO: Converso para GGUF falhou.")
                return False

            # ── ETAPA 5: Resultado ───────────────────────────────────────
            cb(f"\n[5/5] CONCLUDO!")
            cb(f"      Novo modelo em produo: {gguf_final}")
            cb(f"      {'='*50}")
            ...
            if callback:
                callback(f"[Pipeline] ERRO crítico: {e}")
            return False
        finally:
            self._em_execucao = False
    ...
    def _registrar_conclusao(self, alma: str, modelo_base: str, gguf_path: Path):
        ...
        with open(historico_path, "w", encoding="utf-8") as f:
            json.dump(histórico, f, ensure_ascii=False, indent=2)

    def verificar_dependencias(self) -> Dict[str, bool]:
        """Verifica se todas as dependncias de finetuning esto disponíveis."""
        deps = {}
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Pipeline Autnomo de Finetuning  ARCA")
    ...
    parser.add_argument("--verificar", action="store_true", help="Verificar dependncias")
    ...
    if args.todas:
        ...
        for alma, ok in resultados.items():
            print(f"  {'[OK]' if ok else '[ERRO]'} {alma}")
    else:
