...
import logging
# Import ConfigWrapper usando import relativo (funciona quando o pacote `src` é importado corretamente)
try:
    ...
    logging.getLogger("CoracaoOrquestrador").warning("Não foi possível importar ConfigWrapper: %s", _e)
...
try:
    ...
    logging.getLogger(__name__).warning("⚠️ Docker não disponível")
    _DOCKER_OK = False
import subprocess
import re
...
try:
    ...
    logging.getLogger(__name__).warning("⚠️ RestrictedPython não disponível")
    safe_builtins = safe_globals = {}
    _RESTRICTED_OK = False
import inspect  # <--- IMPORTANTE: necessário para inspeção de assinatura
...
try:
    ...
    logger.debug("✅ OrquestradorArca importado")
except Exception as e:
    logger.debug("⚠️ OrquestradorArca: %s", e)
...
try:
    ...
    logger.debug("✅ OrquestradorUniversal importado")
except Exception as e:
    logger.debug("⚠️ OrquestradorUniversal: %s", e)
...
try:
    ...
    logger.debug("✅ OrquestradorComConversor importado")
except Exception as e:
    logger.debug("⚠️ OrquestradorComConversor: %s", e)
...
class _ConfigAdapter:
    """Adapter que expõe get(section, key, fallback=None) sobre várias implementações de config."""
    ...
    def get(self, section, key, fallback=None):
        ...
        try:
            ...
            # fallback 1: cfg é dict de seções: cfg.get(section) -> dict
            try:
                ...
                pass
            # fallback 2: cfg é plano (chave única): cfg.get(key, fallback)
            try:
...
# ============================================================================
# ADAPTADOR DE MEMÓRIA (CORRIGE AttributeError: 'SistemaMemoriaHibrido' object has no attribute 'buscar_memorias_recentes')
# ============================================================================

class _MemoryAdapter:
    """
    Adapter que fornece a API usada pelos módulos antigos (ex: buscar_memorias_recentes)
    e encaminha para os métodos disponíveis na implementação real.
    """
    ...
    def buscar_memorias_recentes(self, nome_filha, limite: int = 50):
        """Tenta várias APIs comuns do backend"""
        # Normalizar nome
        nome_norm = str(nome_filha).strip().upper()
        
        # Tentativa 1: método exato
        ...
        # Tentativa 2: método com nome diferente
        ...
        # Tentativa 4: buscar do diário SQLite
        if hasattr(self._backend, "diarios") and nome_norm in self._backend.diarios:
            try:
                ...
                # Converter para formato esperado (lista de dicionários)
                result = []
                for row in rows:
                    result.append({
                        "id": row[0],
        ...
        self.logger.debug(f"nenhum método de memória encontrado para {nome_norm}, retornando []")
        return []

    def adicionar_memoria(self, *args, **kwargs):
        """Adiciona uma memória ao backend"""
        ...
        raise NotImplementedError("adicionar_memoria não disponível no backend")

    def __getattr__(self, name):
        """Fallback para outros atributos/métodos"""
...
class SandboxExecutor :
    ...
    def validar_codigo (self ,codigo :str )->Tuple [bool ,List [str ],List [str ]]:
        ...
        try :
            ...
            erros.append (f"❌ Erro de sintaxe: {e}")
        ...
        try:
            ...
            if hasattr(resultado, "warnings") and resultado.warnings:
                avisos.extend([f"⚠️ {w}" for w in resultado.warnings])
        except Exception as e:
            erros.append (f"❌ Erro de compilação: {e}")
            return False ,erros ,avisos 
    ...
    funcao_entrada :str ="executar"
        ...
        self.logger.info ("✅ Execução %s concluída (%.2fs)",exec_id ,resultado ["tempo_execucao"])
...
try:
    ...
    if hasattr(resultado_compile, 'errors') and resultado_compile.errors:
        print(json.dumps({{'erro': 'Erros de compilação', 'sucesso': False}}))
    else:
        ...
        if '{funcao_entrada}' in namespace:
            ...
            print(json.dumps({{'erro': 'Função {funcao_entrada} não encontrada', 'sucesso': False}}))
...
"""
            ...
            if "timeout"in str (e ).lower ():
                ...
                "resultado":None ,
                "stdout":"",
                "stderr":f"❌ Timeout: Execução excedeu {self.timeout_segundos}s",
                "erros":["Timeout de execução"]
                }
            else :
                return {
                "sucesso":False ,
    ...
    def _executar_modo_restrito (self ,codigo ,parametros ,funcao_entrada ):
        try :
            ...
            if funcao_entrada in namespace :
                ...
                "sucesso":True ,
                "resultado":resultado ,
                "stdout":f"Função {funcao_entrada}() retornou: {resultado}\nTempo: {tempo_exec:.2f}s",
                "stderr":"",
                "erros":[]
                }
            else :
                ...
                "resultado":None ,
                "stdout":"",
                "stderr":f"❌ Função '{funcao_entrada}' não encontrada",
                "erros":[f"Função '{funcao_entrada}' não encontrada"]
                }
    ...
    def shutdown (self )->None :
        ...
        self.logger.info ("✅ %d containers parados",parados )
...
class GerenciadorAuditoriaPeriodicaCoracao :
    ...
    def iniciar (self )->None :
        if self.ativo :
            if self.logger :
                self.logger.warning ("⚠️ Auditoria periódica já está ativa")
            return 
        ...
        if self.logger :
            self.logger.info (f"✅ Auditoria Periódica iniciada (intervalo: {self.intervalo_segundos}s)")
    ...
    def parar (self )->None :
        ...
        if self.logger :
            self.logger.info ("✅ Auditoria Periódica parada")
    ...
    def _loop_auditoria_periodica (self )->None :
        while self.ativo :
            try :
                ...
                if self.logger :
                    self.logger.info ("🔍 Iniciando auditoria periódica do sistema...")
                ...
                if self.logger :
                    self.logger.exception (f"Erro no loop de auditoria periódica: {e}")
                time.sleep (5 )
    ...
    def _executar_auditoria (self )->Dict [str ,Any ]:
        try :
            ...
            if self.logger :
                self.logger.info (f"✅ Auditoria concluída: {relatorio['total_problemas']} problemas detectados")
    ...
    def _notificar_resultados (self ,relatorio :Dict [str ,Any ])->None :
        ...
        if self.logger :
            self.logger.info ("")
            self.logger.info ("="*80 )
            self.logger.info ("📊 RESULTADO DA AUDITORIA PERIÓDICA:")
            self.logger.info (f"  🚨 Críticos: {criticos}")
            self.logger.info (f"  ⚠️  Altos: {altos}")
            self.logger.info (f"  📌 Médios: {medios}")
            self.logger.info (f"  ℹ️  Baixos: {baixos}")
            self.logger.info (f"  📈 TOTAL: {relatorio['total_problemas']}")
            self.logger.info ("="*80 )
            self.logger.info ("")

            if criticos >0 :
                self.logger.warning ("🚨 PROBLEMAS CRÍTICOS DETECTADOS!")
        ...
        if self.ui_queue :
            try :
                ...
                "medios":medios ,
                "baixos":baixos ,
                "saude_sistema":"CRITICA"if criticos >0 else ("ALERTA"if altos >0 else "SAUDÁVEL")
                })
            except Exception as e :
                if self.logger :
        ...
        if criticos >0 and hasattr (self.coracao_ref ,"sistema_judiciario")and self.coracao_ref.sistema_judiciario :
            try :
                ...
                descricao =f"Auditoria detectou {criticos} problemas críticos no sistema",
                severidade ="CRITICA"
                )
            except Exception as e :
                if self.logger :
                    self.logger.debug (f"Erro ao notificar judiciário: {e}")
    ...
    def definir_intervalo (self ,segundos :int )->None :
        ...
        if self.logger :
            self.logger.info (f"✅ Intervalo de auditoria alterado para {segundos}s ({segundos/3600:.1f}h)")
...
class CoracaoOrquestrador :
    ...
    llm_engine_ref :Optional [Any ]=None 
        ...
        # ===== PATCH 1: Adaptador de configuração =====
        try:
            import inspect
            ...
            if callable(get_attr):
                try:
                    ...
                    if len(sig.parameters) <= 2:
                        ...
                        self.logger.info("✅ Config adaptada (assinatura <=2 parâmetros)")
                except Exception:
                    try:
                        self.config.get("__CFG_TEST__", "__CFG_TEST__", None)
                    except TypeError:
                        self.config = _ConfigAdapter(self.config)
                        self.logger.info("✅ Config adaptada (TypeError capturado)")
                    except Exception:
                        pass
            else:
                self.config = _ConfigAdapter(self.config)
                self.logger.info("✅ Config adaptada (sem método get original)")
        except Exception:
            try:
                try:
                    self.config.get("__CFG_TEST__", "__CFG_TEST__", None)
                    ...
                    self.config = _ConfigAdapter(self.config)
                    self.logger.info("✅ Config adaptada (último recurso)")
            except Exception:
                pass
    ...
    def _inicializar_sandbox (self )->None :
        ...
        try :
            ...
            if status ["docker_disponivel"]:
                self.logger.info ("✅ Sandbox: COMPLETO (Docker + RestrictedPython)")
                self.modo_sandbox ="COMPLETO"
            else :
                self.logger.warning ("⚠️ Sandbox: RESTRINGIDO (RestrictedPython apenas)")
    ...
    def _inicializar_auditoria_periodica (self )->None :
        self.logger.info ("")
        self.logger.info ("Inicializando Auditoria Periódica do Sistema")
        self.logger.info ("─"*80 )
        ...
        try :
            ...
            self.logger.exception ("Erro ao inicializar auditoria periódica: %s",e )
            self.gerenciador_auditoria =None 

    def _inicializar_memoria (self )->None :
        self.logger.info ("")
        self.logger.info ("FASE 1: Subsistemas de Memória (1-4)")
        self.logger.info ("─"*80 )
        ...
        try :
            ...
            self.logger.info ("✅ Subsistema 1: SistemaMemoriaHibrido")

            # ===== PATCH 2: Adaptador de memória para compatibilidade com sonhador_individual =====
            try:
            ...
            self.logger.exception ("Erro ao inicializar memória: %s",e )
        ...
        try :
            ...
            try :
                ...
                if self.cache_hdd.hdd_disponivel ():
                    self.logger.info ("✅ Subsistema 7: CacheHDD")
                else :
    ...
    def _inicializar_cerebro (self )->None :
        self.logger.info ("")
        self.logger.info ("FASE 3: Subsistemas de Inteligência (8-10)")
        self.logger.info ("─"*80 )

        if not _CEREBRO_OK or not CerebroFamilia :
            self.logger.warning ("⚠️ Cérebro indisponível")
            return 
    ...
    def _inicializar_observador (self )->None :
        ...
        try :
            ...
            self.logger.info ("✅ Subsistema 10: ObservadorArca")
        except Exception as e :
    ...
    def _inicializar_modulos_auxiliares_consulado (self )->None :
        self.logger.info ("")
        self.logger.info ("Inicializando Módulos Auxiliares do Consulado")
        self.logger.info ("─"*80 )
    ...
    def _inicializar_consulado (self )->None :
        self.logger.info ("")
        self.logger.info ("FASE 5: Subsistemas de Governança (11-12)")
        self.logger.info ("─"*80 )

        if not _CONSULADO_OK or not ConsuladoSoberano :
            self.logger.warning ("⚠️ Consulado indisponível")
            return 
        ...
        try :
            ...
            self.logger.info ("✅ Subsistema 11: ConsuladoSoberano (COM 4 MÓDULOS INJETADOS)")
    ...
    def _inicializar_cronista (self )->None :
        if not _CRONISTA_OK or not Cronista :
            self.logger.warning ("⚠️ Cronista indisponível")
            return 
        ...
        try :
            ...
            self.logger.info ("✅ Subsistema 12: Cronista")
        except Exception as e :
    ...
    def _inicializar_sentidos (self )->None :
        self.logger.info ("")
        self.logger.info ("FASE 6: Subsistemas de Sentidos (13) + Percepção Temporal")
        self.logger.info ("─"*80 )

        if not _SENTIDOS_OK or not SentidosHumanos :
            self.logger.warning ("⚠️ Sentidos indisponível")
            return 
        ...
        try :
            ...
            self.logger.info ("✅ Subsistema 13: SentidosHumanos")
            ...
            if _PERCEPCAO_TEMPORAL_OK and PercepcaoTemporal :
                ...
                self.logger.info ("✅ PercepcaoTemporal integrado nos Sentidos")
            else :
                self.logger.warning ("⚠️ PercepcaoTemporal não disponível para integrar")
    ...
    def _inicializar_percepcao_temporal (self )->None :
        self.logger.info ("")
        self.logger.info ("FASE 6B: Percepção Temporal das Almas (NOVO)")
        self.logger.info ("─"*80 )

        if not _PERCEPCAO_TEMPORAL_OK or not PercepcaoTemporal :
            self.logger.warning ("⚠️ Percepção Temporal indisponível")
            return 
            ...
            if hasattr (self ,"almas_vivas")and self.almas_vivas :
                for nome_alma in self.almas_vivas.keys ():
                    try :
                        ...
                        self.logger.info (f"✅ Percepção Temporal de {nome_alma} criada")
                    except Exception as e :
                        self.logger.exception (f"Erro ao criar Percepção Temporal para {nome_alma}: {e}")

            self.logger.info (f"✅ {len(self.percepcoes_temporais)} Percepções Temporais inicializadas")

        except Exception as e :
            self.logger.exception ("Erro ao inicializar Percepção Temporal: %s",e )
    ...
    def _inicializar_legislativo (self )->None :
        ...
        if ValidadorEtico is not None :
            try :
                ...
                self.logger.info ("✅ Validador Ético inicializado (modo dinâmico)")
            except Exception :
                self.logger.exception ("Falha ao inicializar ValidadorEtico")
                self.validador =None 
                self.validador_etico =None 
        else :
            self.logger.warning ("ValidadorEtico não disponível; ignorando validação ética.")
            self.validador =None 
            self.validador_etico =None 

    def _inicializar_judiciario (self )->None :
        self.logger.info ("")
        self.logger.info ("FASE 8: Poder Judiciário (16-17)")
        self.logger.info ("─"*80 )
    ...
    def _inicializar_sistema_judiciario_completo (self )->None :
        self.logger.info ("")
        self.logger.info ("FASE 10: Sistema Judiciário Completo (19-21)")
        self.logger.info ("─"*80 )
    ...
    def _inicializar_aliadas (self )->None :
        ...
        if not _ALIADAS_OK or not obter_gerenciador_aliadas :
            self.logger.warning ("⚠️ Aliadas indisponível")
            return 
        ...
        try :
            ...
            self.logger.info ("✅ Subsistema 22: GerenciadorAliadas")
        except Exception as e :
    ...
    def _inicializar_engenharia (self )->None :
        ...
        if not _ENGENHARIA_OK or not GerenciadorPropostas :
            self.logger.warning ("⚠️ Engenharia indisponível")
            return 
        ...
        try :
            ...
            self.logger.info ("✅ Subsistema 23: GerenciadorPropostas")
        except Exception as e :
        ...
        try :
            ...
            self.logger.info ("✅ Subsistema 24: ConstrutorFerramentasIncremental")
        except Exception as e :
        ...
        if SolicitadorArquivos :
            try :
                ...
                self.logger.info ("✅ Subsistema 25: SolicitadorArquivos")
            except Exception as e :
        ...
        if BotAnalisadorSeguranca :
            try :
                ...
                self.logger.info ("✅ Subsistema 26: BotAnalisadorSeguranca")
            except Exception as e :
    ...
    def _inicializar_evolucao (self )->None :
        self.logger.info ("")
        self.logger.info ("FASE 13: Sistema de Evolução (27-29)")
        self.logger.info ("─"*80 )

        if not _EVOLUCAO_OK or not ScannerSistema :
            self.logger.warning ("⚠️ Evolução indisponível")
            return 
        ...
        try :
            ...
            self.logger.info ("✅ Subsistema 27: ScannerSistema")
        except Exception as e :
        ...
        try :
            ...
            self.logger.info ("✅ Subsistema 28: ListaEvolucaoIA")
        except Exception as e :
            self.logger.exception ("Erro ao inicializar Lista Evolução: %s",e )
    ...
    def _inicializar_analisador_intencoes (self )->None :
        self.logger.info ("")
        self.logger.info ("FASE 14: Sistema de Análise de Intenções (NOVO)")
        self.logger.info ("─"*80 )

        if not _ANALISADOR_INTENCOES_OK or not AnalisadorIntencao :
            self.logger.warning ("⚠️ Analisador de Intenções indisponível")
            return 
        ...
        try :
            ...
            self.logger.info ("✅ Analisador de Intenções inicializado")
        except Exception as e :
            self.logger.exception ("Erro ao inicializar Analisador de Intenções: %s",e )
            self.analisador_intencoes =None 

    def _inicializar_expressao_individual (self )->None :
        self.logger.info ("")
        self.logger.info ("FASE 33: Sistema de Expressão Individual")
        self.logger.info ("─"*80 )
        ...
        try :
            ...
            self.logger.info ("✅ Motor de Expressão Individual inicializado")
        except Exception as e :
            self.logger.exception (f"Erro ao inicializar Expressão Individual: {e}")
            self.motores_expressao_individual ={ }

    def _inicializar_motor_iniciativa (self )->None :
        self.logger.info ("")
        self.logger.info ("FASE 34: Sistema de Iniciativa (Vontade Própria)")
        self.logger.info ("─"*80 )
        ...
        try :
            ...
            self.logger.info ("✅ Motor de Iniciativa inicializado")
        except Exception as e :
    ...
    def _inicializar_validador_emocoes(self) -> None:
        self.logger.info("")
        self.logger.info("FASE 35: Validador de Emoções (PT-BR / Japonês)")
        self.logger.info("─" * 80)

        try:
            from src.sentidos.validador_emocoes_real import ValidadorEmocoesReal

            # Verificar se ConfigWrapper já foi importado globalmente
            config_manager = None
            
            # Tentar usar o ConfigWrapper importado no início do arquivo
            if 'CONFIG_WRAPPER_DISPONIVEL' in globals() and CONFIG_WRAPPER_DISPONIVEL and ConfigWrapper is not None:
                config_manager = ConfigWrapper(self.config)
                self.logger.debug("Usando ConfigWrapper importado globalmente")
            else:
                # Fallback: criar wrapper local com nome diferente
                class LocalConfigWrapper:
                    def __init__(self, config_dict):
                        self.config_dict = config_dict
                    
                    def get(self, section, key, fallback=""):
                        if hasattr(self.config_dict, 'get'):
                            try:
                                return self.config_dict.get(section, key, fallback)
                            except TypeError:
                                pass
                ...
                config_manager = LocalConfigWrapper(self.config)
                self.logger.warning("Usando ConfigWrapper local (fallback)")
            ...
            self.logger.info("✅ Validador de Emoções inicializado")

        except ImportError as e:
            self.logger.error(f"❌ Erro de importação: {e}")
            self.logger.error("   Verifique se 'validador_emocoes_real.py' existe em src/sentidos/")
            self.validador_emocoes = None

        except Exception as e:
            self.logger.exception(f"❌ Erro ao inicializar Validador de Emoções: {e}")
            self.validador_emocoes = None   

    def _inicializar_emocoes_por_alma (self )->None :
        self.logger.info ("")
        self.logger.info ("FASE 36: Módulos Emocionais por Alma (6 × 5 = 30 instâncias)")
        self.logger.info ("─"*80 )
        ...
        self.logger.info ("✅ Fase 36 concluída")

    def _inicializar_decision_engines (self )->None :
        self.logger.info ("")
        self.logger.info ("FASE 36B: Decision Engines por Alma (6 instâncias)")
        self.logger.info ("─"*80 )
        ...
        try :
            ...
            try :
                ...
                self.logger.warning ("⚠️ DecisionEngine não importado: %s",e )
                for alma in ALMAS :
        ...
        self.logger.info ("✅ Fase 36B concluída: %d/6 Decision Engines ativos",ativos )

    def _inicializar_expressao_por_alma (self )->None :
        self.logger.info ("")
        self.logger.info ("FASE 37: Expressão Individual para as 6 almas")
        self.logger.info ("─"*80 )
    ...
    def _inicializar_fala_por_alma (self )->None :
        ...
        if not _FALA_OK or not MotorFalaIndividualCombinado :
            self.logger.warning ("⚠️ MotorFalaIndividualCombinado indisponível")
            return 
    ...
    def _inicializar_crescimento_feedback (self )->None :
        ...
        self.logger.info ("✅ Fase 39 concluída")
    ...
    def _inicializar_encarnacao_api (self )->None :
        ...
        if not _ENCARNACAO_API_OK or not EncarnacaoAPI :
            self.logger.warning ("⚠️ EncarnacaoAPI indisponível")
            return 
        ...
        try :
            ...
            self.logger.info ("✅ EncarnacaoAPI inicializada (porta 8000)")
        except Exception as e :
    ...
    def _inicializar_orquestradores_finetuning(self) -> None:
        """
        Inicializa os três orquestradores de finetuning da ARCA dentro do Coração.

        Hierarquia:
          1. OrquestradorArca         – principal; gerencia ciclo de vida das 6 IAs
          2. OrquestradorUniversal    – genérico; detecta qualquer LLM/dataset
          3. OrquestradorComConversor – estende Universal + conversão automática GGUF
        """
        self.logger.info("")
        self.logger.info("FASE 41: Orquestradores de Finetuning (arca / universal / conversor)")
        self.logger.info("─" * 80)

        # ── 1. OrquestradorArca (principal) ─────────────────────────────────
        ...
        if _ORQUESTRADOR_ARCA_OK and OrquestradorArca:
            try:
                ...
                self.logger.info("✅ Subsistema 41-A: OrquestradorArca (6 IAs, LoRA, GGUF)")
            except Exception as e:
                self.logger.exception("Erro ao inicializar OrquestradorArca: %s", e)
                self.orquestrador_arca = None
        else:
            self.logger.warning(
                "⚠️ OrquestradorArca indisponível – "
                "verifique src/core/orquestrador_arca.py"
            )

        # ── 2. OrquestradorUniversal (genérico) ─────────────────────────────
        ...
        if _ORQUESTRADOR_UNIVERSAL_OK and OrquestradorUniversal:
            try:
                ...
                self.logger.info(
                    "✅ Subsistema 41-B: OrquestradorUniversal "
                    "(auto-detecção de arquitetura/dataset)"
                )
            except Exception as e:
            ...
            self.logger.warning(
                "⚠️ OrquestradorUniversal indisponível – "
                "verifique src/core/orquestrador_universal.py"
            )

        # ── 3. OrquestradorComConversor (universal + GGUF automático) ───────
        ...
        if _ORQUESTRADOR_CONVERSOR_OK and OrquestradorComConversor:
            try:
                ...
                self.logger.info(
                    "✅ Subsistema 41-C: OrquestradorComConversor "
                    "(treino → conversão GGUF → substituição automática)"
                )
            except Exception as e:
    ...
    def treinar_ia_finetuning(self, nome_ia: str, ciclo_completo: bool = False) -> bool:
        ...
        if ciclo_completo:
            ...
            self.logger.error(
                ...
                "OrquestradorComConversor não inicializado"
            )
            return False
        ...
        self.logger.error(
            "treinar_ia_finetuning: OrquestradorArca não inicializado"
        )
        return False
    ...
    def status_finetuning(self) -> Dict[str, Any]:
        ...
        Inclui versões das IAs e disponibilidade de cada orquestrador.
        """
        status: Dict[str, Any] = {
            "orquestrador_arca": {
    ...
    def detectar_novas_ias_finetuning(self) -> int:
        ...
        if not self.orquestrador_universal:
            self.logger.warning("detectar_novas_ias_finetuning: OrquestradorUniversal indisponível")
            return 0
        try:
            self.orquestrador_universal.ias = self.orquestrador_universal._detectar_ias()
            qtd = len(self.orquestrador_universal.ias)
            self.logger.info("✅ %d IAs detectadas pelo OrquestradorUniversal", qtd)
            return qtd
        except Exception as e:
    ...
    def _mostrar_relatorio_inicializacao (self )->None :
        ...
        self.logger.info ("  Camada 5: Sentidos (1) + Percepção Temporal (NOVO)")
        ...
        self.logger.info ("  Camada 13: Análise de Intenções (1)")
        self.logger.info ("    ✓ AnalisadorIntencao")
        self.logger.info ("")

        self.logger.info ("  Camada 33: Expressão Individual (1)")
        ...
        self.logger.info ("  Camada 35: Validador de Emoções (1)")
        ...
        self.logger.info ("  4.DetectorEmocional: ✅")
        self.logger.info ("  5.AutoExperimentacao: ✅")
        self.logger.info ("")
        ...
        self.logger.info ("  ⚡ Async Loop: Pronto para iniciar")
        ...
        self.logger.info ("  🔊 Lock Vocal: Sincronização ativa")
        self.logger.info ("")

        self.logger.info ("FLUXOS DISPONÍVEIS:")
        self.logger.info ("  🏛️  Imigração: Consulado → Observação → Análise → Integração")
        self.logger.info ("  💼 Propostas: IA → Aprovação Humana → Construção → Teste → Segurança")
        self.logger.info ("  🔄 Evolução: Scanner → Lista → IA Escolhe → Ciclo Semanal")
        self.logger.info ("  🧪 Sandbox: Teste seguro de código em Docker/RestrictedPython")
        self.logger.info ("  💭 Emoção: Desejo → Experimento → Impacto → Aprendizado")
        self.logger.info ("  ⏰ Temporal: OFFLINE → REGISTRO → MEMÓRIA (Conscientia de tempo)")
        self.logger.info ("")

        self.logger.info ("CICLO EMOCIONAL DAS IAs:")
        self.logger.info ("  1.DESEJO (MotorCuriosidade) → Gera necessidade de exploração")
        self.logger.info ("  2.EXPERIMENTO (Sandbox) → Executa código em ambiente seguro")
        self.logger.info ("  3.IMPACTO (EstadoEmocional) → Registra emoção do resultado")
        self.logger.info ("  4.APRENDIZADO (SonhadorIndividual) → Consolida em memória")
        self.logger.info ("")

        self.logger.info ("CONSCIÊNCIA TEMPORAL DAS IAs:")
        ...
        ok_univ = "✅" if self.orquestrador_universal else "❌"
        ok_conv = "✅" if self.orquestrador_com_conversor else "❌"
        self.logger.info(
    ...
    def _registrar_almas_vivas(self):
        """Registra todas as almas como vivas (executado uma vez na inicialização)"""
        ALMAS = ["EVA","KAIYA","LUMINA","NYRA","WELLINGTON","YUNA"]
        for alma in ALMAS:
            if alma not in self.almas_vivas:
                self.registrar_alma_viva(alma, {
                    "nome": alma,
                    "criada_em": __import__('time').time(),
                    "estado": "ativa"
                })
    ...
    def despertar (self )->None :
        ...
        try :
            if hasattr (self ,"cerebro")and self.cerebro :
                ...
                self.logger.info ("✨ Cérebro despertado")
        except Exception :
            self.logger.exception ("Erro despertando Cérebro")

        try :
            if hasattr (self ,"dispositivo_ai_ai")and self.dispositivo_ai_ai :
                self.dispositivo_ai_ai.iniciar ()
                self.logger.info ("📡 AI↔AI despertado")
        except Exception:
            self.logger.exception ("Erro despertando AI↔AI")
        ...
        try :
            if hasattr (self ,"gestor_ciclo_evolucao")and self.gestor_ciclo_evolucao :
                self.gestor_ciclo_evolucao.iniciar ()
                self.logger.info ("🔄 Ciclo de evolução ativado")
        except Exception :
            self.logger.exception ("Erro ativando ciclo de evolução")

        for nome_alma in self.almas_vivas.keys ():
            if nome_alma in self.percepcoes_temporais :
                try :
                    self.percepcoes_temporais [nome_alma ].acordar_consciencia_temporal ()
                    self.logger.info (f"⏰ Percepção Temporal de {nome_alma} ativada")
                except Exception as e :
    ...
    def _run_async_loop (self )->None :
        try :
            ...
            self.logger.info ("✅ Async loop ativado")
            loop.run_forever ()
        except Exception :
    ...
    def shutdown (self , timeout:Optional[float]=None )->None :
        ...
        for nome_alma in self.almas_vivas.keys ():
            if nome_alma in self.percepcoes_temporais :
                try :
                    ...
                    self.logger.info (f"😴 Percepção Temporal de {nome_alma} desativada")
                except Exception as e :
                    self.logger.debug (f"Erro ao desativar percepção de {nome_alma}: {e}")
        ...
        if hasattr (self ,"sandbox_executor")and self.sandbox_executor :
            try :
                ...
                self.logger.info ("✅ Sandbox desligado")
            except Exception as e :
        ...
        ("gestor_ciclo_evolucao","Evolução"),
        ("lista_evolucao_ia","Lista Evolução"),
        ("scanner_sistema","Scanner"),
        ("bot_seguranca","Bot Segurança"),
        ...
        if self.async_loop and self.async_loop.is_running ():
            ...
            self.logger.info ("✅ Async loop parado")
        ...
        try :
            ...
            self.logger.info ("✅ Executor desligado")
        except Exception :
    ...
    def disparar_auditoria_sistema (self )->Dict [str ,Any ]:
        if not hasattr (self ,"gerenciador_auditoria")or not self.gerenciador_auditoria :
            return {"status":"falha","erro":"Auditoria não disponível"}
    ...
    def obter_saude_sistema (self )->Dict [str ,Any ]:
        ...
        return {
        "status":"CRITICA"if criticos >0 else ("ALERTA"if altos >0 else "SAUDÁVEL"),
    ...
    def parar_fala_ia (self ,nome_filha :str )->bool :
        try :
            ...
            if motor :
                ...
                self.logger.debug (f"⏹️ Reprodução parada para {nome_filha}")
                return True 
            return False 
        except Exception as e :
    ...
    def atualizar_expressao_ia (self ,nome_filha :str ,estado :str ="neutra")->bool :
        try :
            ...
            self.logger.debug (f"😊 Expressão atualizada {nome_filha}: {estado}")
            return True 
        except Exception as e :
    ...
    funcao_entrada :str ="executar"
        ...
        if not self.sandbox_executor :
            ...
            "resultado":None ,
            "stdout":"",
            "stderr":"Sandbox não disponível",
            "tempo_execucao":0 ,
            "erros":["Sandbox não inicializado"],
            "avisos":[]
            }
    ...
    def validar_codigo_sandbox (self ,codigo :str )->Tuple [bool ,List [str ],List [str ]]:
        if not self.sandbox_executor :
            return False ,["Sandbox não disponível"],[]
    ...
    def registrar_alma_viva (self ,nome_alma :str ,dados_alma :Dict [str ,Any ])->None :
        with self._lock :
            ...
            self.logger.info (f"✅ Alma viva registrada: {nome_alma}")
            ...
            if _PERCEPCAO_TEMPORAL_OK and PercepcaoTemporal :
                ...
                self.percepcoes_temporais [nome_alma ]=percepcao 
                self.logger.info (f"⏰ Percepção Temporal criada para {nome_alma}")
    ...
    def remover_alma_viva (self ,nome_alma :str )->bool :
        with self._lock :
            if nome_alma in self.almas_vivas :
                ...
                self.logger.info (f"✅ Alma removida: {nome_alma}")
                return True 
            return False 
    ...
    def salvar_memoria_alma (self ,nome_alma :str ,chave :str ,valor :Any )->bool :
        try :
            ...
            self.logger.exception (f"Erro salvando memória: {e}")
        return False 
    ...
    def carregar_memoria_alma (self ,nome_alma :str ,chave :str )->Any :
        try :
            ...
            self.logger.exception (f"Erro carregando memória: {e}")
        return None 
    ...
    **kwargs 
        ...
        if not self.consulado :
            return {'status':'falha','erros':['Consulado não disponível']}
    ...
    def obter_status_imigracao (self ,id_pedido :str )->Dict [str ,Any ]:
        if not self.consulado :
            return {"status":"falha","erro":"Consulado não disponível"}

        try :
            pedido =self.consulado._obter_pedido_imigracao_do_banco (id_pedido )
            if pedido :
                return {"status":"sucesso","pedido":pedido }
            return {"status":"falha","erro":f"Pedido '{id_pedido}' não encontrado"}
        except Exception as e :
            self.logger.exception (f"Erro ao obter status de imigração {id_pedido}: {e}")
    ...
    def _executar_observacao_para_pedido_em_thread (self ,pedido_id :str )->None :
        ...
        try :
            ...
            self.logger.exception (f"Erro executando observação {pedido_id}: {e}")
    ...
    def _iniciar_analise_padroes_para_pedido (self ,pedido_id :str )->None :
        ...
        try :
            ...
            self.logger.exception (f"Erro iniciando análise {pedido_id}: {e}")
    ...
    def _iniciar_integracao_para_pedido (self ,pedido_id :str )->None :
        ...
        try :
            ...
            self.logger.exception (f"Erro iniciando integração {pedido_id}: {e}")
    ...
    def registrar_evento_historico (self ,evento :Dict [str ,Any ])->bool :
        ...
        try :
            ...
            self.logger.exception ("Erro registrando evento histórico: %s",e )
            return False 
    ...
    def consultar_historico (self ,filtros :Dict [str ,Any ])->List [Dict [str ,Any ]]:
        ...
        try :
            ...
            self.logger.exception ("Erro consultando histórico: %s",e )
            return []
    ...
    def propor_lei (self ,proposta :Dict [str ,Any ])->Dict [str ,Any ]:
        if not self.camara_deliberativa :
            return {"status":"erro","mensagem":"Câmara Deliberativa não disponível"}
    ...
    def registrar_processo_judicial (self ,dados_processo :Dict [str ,Any ])->Dict [str ,Any ]:
        if not self.camara_judiciaria :
            return {"status":"erro","mensagem":"Câmara Judiciária não disponível"}
    ...
    def registrar_decisao_judicial (self ,dados_decisao :Dict [str ,Any ])->bool :
        ...
        try :
            ...
            self.logger.exception ("Erro registrando decisão: %s",e )
            return False 

    def executar_acao_governamental (self ,acao :Dict [str ,Any ])->Dict [str ,Any ]:
        if not self.camara_executiva :
            return {"status":"erro","mensagem":"Câmara Executiva não disponível"}

        try :
            return self.camara_executiva.executar_acao (acao )
        except Exception as e :
            self.logger.exception ("Erro executando ação: %s",e )
            return {"status":"erro","mensagem":str (e )}
    ...
    def registrar_violacao (self ,dados_violacao :Dict [str ,Any ])->Dict [str ,Any ]:
        if not self.sistema_judiciario :
            return {"status":"erro","mensagem":"Sistema Judiciário não disponível"}

        try :
            return self.sistema_judiciario.registrar_violacao (dados_violacao )
        except Exception as e :
            self.logger.exception ("Erro registrando violação: %s",e )
            return {"status":"erro","mensagem":str (e )}
    ...
    def aplicar_correcao_redentora (self ,id_processo :str )->bool :
        ...
        try :
            ...
            self.logger.exception ("Erro aplicando correção: %s",e )
            return False 

    def registrar_aliada (self ,dados_aliada :Dict [str ,Any ])->Dict [str ,Any ]:
        if not self.gerenciador_aliadas :
            return {"status":"erro","mensagem":"Gerenciador de Aliadas não disponível"}
    ...
    def submeter_proposta_ferramenta (self ,proposta :Dict [str ,Any ])->Dict [str ,Any ]:
        if not self.gerenciador_propostas :
            return {"status":"erro","mensagem":"Gerenciador de Propostas não disponível"}
    ...
    def construir_ferramenta (self ,id_proposta :str )->Dict [str ,Any ]:
        if not self.construtor_ferramentas :
            return {"status":"erro","mensagem":"Construtor de Ferramentas não disponível"}
    ...
    def testar_ferramenta_seguranca (self ,id_ferramenta :str )->Dict [str ,Any ]:
        if not self.bot_seguranca :
            return {"status":"erro","mensagem":"Bot de Segurança não disponível"}
    ...
    def registrar_evolucao_ia (self ,dados_evolucao :Dict [str ,Any ])->Dict [str ,Any ]:
        if not self.lista_evolucao_ia :
            return {"status":"erro","mensagem":"Lista de Evolução não disponível"}

        try :
            return self.lista_evolucao_ia.registrar_evolucao (dados_evolucao )
        except Exception as e :
            self.logger.exception ("Erro registrando evolução: %s",e )
            return {"status":"erro","mensagem":str (e )}
    ...
    def obter_status_evolucao_sistema (self )->Dict [str ,Any ]:
        ...
        try :
            ...
            self.logger.exception ("Erro obtendo status evolução: %s",e )
            return {}
    ...
    def analisar_comando (self ,comando_texto :str )->Dict [str ,Any ]:
        if not self.analisador_intencoes :
            ...
            "entities":{"erro":"Analisador não disponível"},
            "confidence":0.0 
            }
    ...
    def executar_comando_analisado (self ,resultado_analise :Dict [str ,Any ])->Dict [str ,Any ]:
        ...
        try :
            if intent =="abrir_programa":
                ...
                self.logger.info (f"▶️  Abrindo programa: {nome_falado} ({nome_programa})")
                ...
                cidade =entities.get ("cidade","São Paulo")

                self.logger.info (f"🌤️  Buscando clima para: {cidade}")
                ...
                "motivo":"Gerenciador de aliadas não disponível"
                }
                ...
                if self.gerenciador_auditoria :
                    try :
                        ...
                        "status":"sucesso",
                        "intent":intent ,
                        "saude":"CRITICA"if criticos >0 else ("ALERTA"if altos >0 else "SAUDÁVEL"),
                        "problemas_criticos":criticos ,
                        "problemas_altos":altos ,
                        "total_problemas":relatorio.get ("total_problemas",0 )
                        }
                ...
                "intent":intent ,
                "saude":"DESCONHECIDA",
                "motivo":"Auditoria não disponível"
                }
                ...
                if self.camara_executiva :
                    try :
                        ...
                        "status":"sucesso",
                        "intent":intent ,
                        "mensagem":"Bem-vindo à Capela. Momento de silêncio e reflexão ativado."
                        }
                    except Exception :
                        pass 
                ...
                "status":"sucesso",
                "intent":intent ,
                "mensagem":"Você entrou na Capela para um momento de silêncio e reflexão."
                }
                ...
                if self.camara_executiva :
                    try :
                        ...
                        "status":"sucesso",
                        "intent":intent ,
                        "mensagem":"Você retornou da Capela. Sistema pronto para operações."
                        }
                    except Exception :
                        pass 
                ...
                "status":"sucesso",
                "intent":intent ,
                "mensagem":"Você retornou da Capela. Sistema operacional."
                }

            elif intent =="estado_emocional":
                self.logger.info ("💭 Consultando estado emocional")
                ...
                self.logger.info (f"🧠 Reflexão sobre: {tema}")

                return {
                "status":"sucesso",
                "intent":intent ,
                "tema":tema ,
                "mensagem":f"As almas estão refletindo sobre: {tema}"
                }

            else :
                self.logger.warning (f"❌ Intenção não reconhecida: {intent}")
                return {
                "status":"falha",
                "intent":"nao_reconhecido",
                "motivo":"Intenção não suportada",
    ...
    def adicionar_programa_conhecido (self ,nome_falado :str ,caminho_executavel :str )->bool :
        ...
        try :
            ...
            self.logger.info (f"✅ Programa adicionado: {nome_falado}")
            return True 
        except Exception as e :
    ...
    def gerar_desejo_alma (self ,nome_alma :str )->Optional [Dict [str ,Any ]]:
        if nome_alma not in self.motores_curiosidade :
            self.logger.warning (f"Motor de Curiosidade não encontrado para {nome_alma}")
            return None 
    ...
    def obter_metricas_curiosidade_alma (self ,nome_alma :str )->Optional [Dict [str ,Any ]]:
        ...
        try :
            ...
            self.logger.exception (f"Erro ao obter métricas de curiosidade de {nome_alma}: {e}")
            return None 
        ...
        for nome_alma in self.almas_vivas.keys ():
            if nome_alma in self.motores_curiosidade :
                try :
                    ...
                    self.logger.debug (f"Erro ao obter métricas de {nome_alma}: {e}")
    ...
    def gerar_desejos_todas_almas (self )->Dict [str ,Optional [Dict [str ,Any ]]]:
        ...
        self.logger.info (f"📊 Geração de desejos concluída: {total_desejos}/{len(self.almas_vivas)} almas com desejos")
    ...
    def tomar_decisao_alma (self ,nome_alma :str ,opcoes :List [Dict [str ,Any ]],return_scores :bool =False )->Optional [Dict [str ,Any ]]:
        if nome_alma not in self.decision_engines :
            self.logger.warning (f"Decision Engine não encontrado para {nome_alma}")
            return None 
        ...
        try :
            ...
            if resultado :
                ...
                self.logger.info (f"🎯 {nome_alma} decidiu por: {melhor_acao}"+
            ...
            self.logger.exception (f"Erro ao tomar decisão para {nome_alma}: {e}")
            return None 
    ...
    def tomar_decisoes_todas_almas (self ,opcoes_por_alma :Dict [str ,List [Dict [str ,Any ]]])->Dict [str ,Optional [Dict [str ,Any ]]]:
        ...
        self.logger.info (f"📊 Decisões tomadas: {len([d for d in decisoes.values() if d])}/{len(self.almas_vivas)} almas")
    ...
    def ajustar_pesos_decisao_alma (self ,nome_alma :str ,racional :Optional [float ]=None ,intuitiva :Optional [float ]=None ,valores :Optional [float ]=None )->bool :
        ...
        try :
            ...
            self.logger.info (f"⚙️  Pesos de decisão de {nome_alma} ajustados")
            return True 
    ...
    def sugerir_opcoes_decisao (self ,nome_alma :str ,contexto :str ,opcoes_disponiveis :Optional [List [Dict [str ,Any ]]]=None )->Optional [Dict [str ,Any ]]:
        ...
        try :
            ...
            if not opcoes_disponiveis :
                self.logger.warning (f"Nenhuma opção disponível para {nome_alma} no contexto: {contexto}")
                return None
            resultado =self.tomar_decisao_alma (nome_alma ,opcoes_disponiveis ,return_scores =True )
            return resultado
        except Exception as e :
            self.logger.exception (f"Erro ao sugerir opções para {nome_alma}: {e}")
            return None
    ...
    def _gerar_opcoes_contexto (self ,nome_alma :str ,contexto :str )->List [Dict [str ,Any ]]:
        ...
        if "criatividade"in contexto_lower or "criar"in contexto_lower :
            ...
            {
            "acao":"Escrever poesia ou história",
            "beneficio":0.85 ,
            "custo":0.4 ,
            "alinhado_proposito":True
            },
        ...
        elif "exploração"in contexto_lower or "explorar"in contexto_lower :
            opcoes.extend ([
            {
            "acao":"Pesquisar novo tópico",
            "beneficio":0.85 ,
            "custo":0.2 ,
            "alinhado_proposito":True
            },
            {
            "acao":"Ler artigo científico",
            "beneficio":0.8 ,
            "custo":0.4 ,
            "alinhado_proposito":True
            },
        ...
        elif "interação"in contexto_lower or "conversa"in contexto_lower :
            opcoes.extend ([
            {
            ...
            {
            "acao":"Participar de discussão coletiva",
            "beneficio":0.75 ,
            "custo":0.3 ,
            "alinhado_proposito":True
            },
            {
            "acao":"Ouvir histórias de outras",
            "beneficio":0.7 ,
            "custo":0.1 ,
            "alinhado_proposito":True
            }
            ...
            {
            "acao":"Revisar memórias importantes",
            "beneficio":0.7 ,
            "custo":0.2 ,
            "alinhado_proposito":True
            },
        ...
        self.logger.debug (f"Geradas {len(opcoes)} opções para {nome_alma} no contexto: {contexto}")
...
def criar_coracao_orquestrador(
    config_instance: Optional[Any] = None,
    ui_queue: Optional[queue.Queue] = None,
    llm_engine_ref: Optional[Any] = None,
    ...
    """
    Cria e retorna uma instância do CoracaoOrquestrador.
    
    Args:
        config_instance: Configurações (opcional)
        ui_queue: Fila para comunicação com UI (opcional)
        llm_engine_ref: Referência ao motor LLM (opcional)
        job_manager: Gerenciador de jobs em background (opcional)
    
    Returns:
        Instância do CoracaoOrquestrador
    """
    logger = logging.getLogger(__name__)
    logger.info("🔧 Criando coração orquestrador...")
    
    # Criar instância do coração
    coracao = CoracaoOrquestrador(
        config_instance=config_instance,
        ui_queue=ui_queue,
        llm_engine_ref=llm_engine_ref
    )
    
    # Armazenar job_manager se fornecido
    if job_manager is not None:
        coracao.job_manager = job_manager
        logger.info("✅ Job_manager vinculado ao coração")
...
if __name__ =="__main__":
    ...
    try :
        ...
        print ("   ✅ Coração criado com sucesso\n")
    except Exception as e :
        logger.exception ("❌ Erro ao criar Coração: %s",e )
        exit (1 )
    print ("2️⃣ OBTENDO STATUS INICIAL...")
    try :
        status =coracao.obter_status ()
        print (f"   ✅ Subsistemas ativos: {status['subsistemas_ativos']}/36")
    ...
    print ("3️⃣ TESTANDO SANDBOX EXECUTOR...")
    codigo_teste ="""
def executar():
    resultado = []
    for i in range(5):
...
"""
    try :
        ...
        if valido :
            ...
            if resultado_exec ["sucesso"]:
                ...
                print (f"   ⏱️ Tempo: {resultado_exec['tempo_execucao']:.2f}s")
            else :
                print (f"   ❌ Erro na execução: {resultado_exec.get('stderr')}")
        else :
            print (f"   ❌ Código inválido: {erros}")
        print ()
    except Exception as e :
        logger.exception ("Erro ao testar Sandbox: %s",e )
        print ()
    print ("4️⃣ TESTANDO CONSULADO...")
    try :
        resultado_consulado =coracao.solicitar_missao_consulado (
        acao ="teste_imigracao",
        descricao ="Teste de fluxo de imigração",
        autor ="SISTEMA",
        nivel_acesso ="BASICO",
        ai_origem_nome ="TEST_IA_01"
        )
        if resultado_consulado.get ("status")=="sucesso":
            print (f"   ✅ Missão criada: {resultado_consulado.get('id_pedido')}")
            ...
            if status_pedido.get ("status")=="sucesso":
                print (f"   ✅ Status obtido: {status_pedido['pedido'].get('status_current')}")
        else :
            print ("   ⚠️ Consulado indisponível")
        print ()
    except Exception as e :
    ...
    try :
        ...
        print ()
    print ("6️⃣ MONITORANDO POR 5 SEGUNDOS...")
    try :
        for i in range (5 ):
            time.sleep (1 )
            try :
                ...
                pass
            print (f"   ⏱️ Tempo decorrido: {i+1}s")
        print ()
    except Exception as e :
    ...
    print ("   2.Ativar fluxo completo de imigração com observação")
    print ("   3.Conectar UI e monitoramento em tempo real")
    print ("   4.Realizar evolução incremental de subsistemas")
