...
"""
ARCA CELESTIAL GENESIS - CARREGADOR SISTÊMICO (AUTODESCOBERTA)
Local: src/core/carregador_sistemico.py

Funo: Escanear diretórios autorizados, importar módulos dinamicamente e registrar no Corao.Melhorias adicionadas:
- CLI para execução independente.
- Filtros custom (override ignores).
- Cache de estado para reloads rpidos.
- Integrao com validador_etico.
...
class CarregadorSistemico:
    ...
    def __init__(self, coracao_ref: Any, diretorios_alvo: Optional[List[str]] = None, custom_ignore: Optional[Set[str]] = None):
        ...
        # garante que coracao possui dicionrio de módulos
        if not hasattr(self.coracao, "modulos") or not isinstance(getattr(self.coracao, "modulos"), dict):
            try:
                setattr(self.coracao, "modulos", {})
            except Exception:
                logger.warning("CarregadorSistemico: no foi possível inicializar coracao.modulos; registro local apenas.")
    ...
    def _validar_modulo_etico(self, caminho_arquivo: Path) -> bool:
        """Integrao com validador_etico se disponível."""
        try:
            ...
            pass
        return True  # assume ok se no disponível

    def escanear_e_conectar(self) -> int:
        """
        Varre os diretórios autorizados e carrega módulos registrveis.Retorna o número de módulos carregados com sucesso.
        """
        logger.info(" Iniciando varredura sistmica em %d setores...", len(self.diretorios_alvo))
        ...
        for diretorio in self.diretorios_alvo:
            ...
            if not path_obj.exists():
                logger.debug("Diretório alvo no existe, ignorando: %s", str(path_obj))
                continue

            for arquivo in path_obj.rglob("*.py"):
                stats["tentados"] += 1
                # Ignorar arquivos e diretórios bvios
                ...
                if "utils" in arquivo.parts or arquivo.match("**/utils/**"):
                    logger.debug("Ignorando utilitrio: %s", arquivo)
                    stats["ignorados"] += 1
                    continue

                # Validao tica
                if not self._validar_modulo_etico(arquivo):
                    logger.warning("Módulo rejeitado por validao tica: %s", arquivo)
                    stats["ignorados"] += 1
                    continue
        ...
        self._salvar_cache()
        logger.info(" CONEXO MASSIVA CONCLUDA: %d módulos carregados.", total_carregados)
        logger.info(" Estatsticas: %s", stats)
        return total_carregados

    def _qualificar_nome_modulo(self, caminho_arquivo: Path) -> str:
        """Constri nome qualificado."""
        try:
    ...
    def _carregar_modulo(self, caminho_arquivo: Path) -> bool:
        ...
        if nome_qualificado in self.modulos_carregados:
            logger.debug("Módulo j carregado (pulando): %s", nome_qualificado)
            try:
        ...
        try:
            ...
            if spec is None or spec.loader is None:
                logger.error("Spec invlido para %s (ignorado)", caminho_arquivo)
                return False
            ...
            try:
                ...
                logger.debug("No foi possível registrar módulo no coracao.modulos; mantendo registro local.")

            logger.info("   -> Módulo carregado: %s (%s)", nome_qualificado, caminho_arquivo)
            return True

        except Exception as e:
            logger.error("[ERRO] Erro ao carregar módulo %s: %s\n%s", caminho_arquivo, e, traceback.format_exc())
            try:
...
def main():
    """CLI para execução independente."""
    parser = argparse.ArgumentParser(description="Carregador Sistmico - Autodescoberta de Módulos")
    ...
    parser.add_argument("--no-cache", action="store_true", help="No usar cache")
    parser.add_argument("--report", action="store_true", help="Mostrar relatrio final")
