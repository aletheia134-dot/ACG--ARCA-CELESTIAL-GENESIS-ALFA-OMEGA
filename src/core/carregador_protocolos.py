...
class Cerebro:
    ...
    def carregar_modelo(self) -> bool:
        """Tenta conectar ao LLM local via llm_client ou servidor_core (porta 5002)."""
        try:
            # Tentativa 1: importar LLMClient local
            try:
                import importlib, sys
                for mod_path in ["llm_client", "src.core.llm_client"]:
                    try:
                        mod = importlib.import_module(mod_path)
                        LLMClient = getattr(mod, "LLMClient", None)
                        if LLMClient:
                            self._llm_client = LLMClient()
                            self.logger.info("[OK] Cérebro — LLMClient carregado de %s", mod_path)
                            self.modelo_carregado = True
                            return True
                    except Exception:
                        continue
            except Exception as e:
                self.logger.debug("LLMClient não disponível: %s", e)

            # Tentativa 2: verificar servidor_core na porta 5002
            import requests as _req
            resp = _req.get("http://localhost:5002/status", timeout=3)
            if resp.status_code == 200:
                self._llm_endpoint = "http://localhost:5002"
                self.logger.info("[OK] Cérebro — servidor_core detectado em 5002")
                self.modelo_carregado = True
                return True

        except Exception as e:
            self.logger.warning("Servidor LLM (5002) não acessível: %s", e)

        self.logger.error(
            "[ERRO] Cérebro sem LLM disponível. "
            "Inicie o servidor_core ou instale llm_client."
        )
        self.modelo_carregado = False
        return False

    def pensar(self, prompt_sistema: str, mensagem_usuario: str, max_tokens: int = 512) -> str:
        """Gera resposta via LLM real (llm_client ou servidor_core)."""
        if not self.circuit_breaker.pode_chamar():
            return "Cérebro indisponível (circuit breaker aberto)"

        if not self.modelo_carregado:
            return "Erro: Cérebro não carregado — execute carregar_modelo() primeiro"

        import concurrent.futures as _cf
        try:
            self.metricas["chamadas"] += 1

            # Usar LLMClient direto
            if hasattr(self, "_llm_client") and self._llm_client:
                with _cf.ThreadPoolExecutor(max_workers=1) as ex:
                    ...
                    resposta = fut.result(timeout=self.timeout)
                texto = str(resposta).replace("<|assistant|>", "").replace("<|im_end|>", "").strip()
                self.circuit_breaker.registrar_sucesso()
                return texto

            # Usar servidor_core via HTTP
            if hasattr(self, "_llm_endpoint") and self._llm_endpoint:
                import requests as _req
                resp = _req.post(
                    f"{self._llm_endpoint}/gerar",
                    json={
                        "system": prompt_sistema,
                ...
                if resp.status_code == 200:
                    dados = resp.json()
                    texto = dados.get("resposta") or dados.get("text") or str(dados)
                    self.circuit_breaker.registrar_sucesso()
                    return texto.strip()
                self.circuit_breaker.registrar_falha()
                return f"Servidor LLM retornou HTTP {resp.status_code}"

            self.circuit_breaker.registrar_falha()
            return "Erro: nenhum backend LLM disponível"

        except _cf.TimeoutError:
            self.circuit_breaker.registrar_falha()
            self.metricas["timeouts"] += 1
            return f"Timeout: LLM não respondeu em {self.timeout}s"
        except Exception as e:
            self.circuit_breaker.registrar_falha()
            self.metricas["erros"] += 1
            self.logger.exception("Erro ao chamar LLM em pensar()")
