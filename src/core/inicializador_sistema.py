...
# ============================================================================
# FUNÇÕES DE LIMPEZA DE MARKDOWN (mantidas dos originais)
# ============================================================================
...
def remove_markdown_blocks_main(dry_run: bool = False) -> int:
    if not SRC.exists():
        print("ERRO: diretório src/ no encontrado.Rode na raiz do projeto.")
        return 1
    ...
    for f in py_files:
        try:
            if dry_run:
                ...
                if removed:
                    print("SUPOSTA ALTERAO:", f)
                continue
            process_file_remove_md(f, timestamp, backups, modified)
        except Exception as e:
            print("Erro processando", f, ":", e)
    if not modified:
        print("Nenhuma alterao necessária (nenhum bloco Markdown encontrado).")
        return 0
    print("Arquivos alterados (backups criados):")
    for m in modified:
        print(" -", m)
    print("\nTestando compilao de todos os.py em src/ ...")
    failed = False
    errors = []
    for f in py_files:
        try:
            ...
            errors.append((f, e))
    if failed:
        print("\nCOMPILAO FALHOU.Restaurando backups...")
        ...
        for f, e in errors:
            print(" -", f, ":", e)
        print("\nArquivos restaurados.Abra os arquivos listados e remova manualmente os blocos Markdown problemticos.")
        return 1
    print("\nCompilao OK aps remoo de bloc Markdown.")
    return 0

# ============================================================================
# IMPORTS PRINCIPAIS (unificando as melhores versões)
# ============================================================================
...
try:
    ...
    # ===== IMPORT CORRIGIDO DO GERENCIADOR DE SEGREDOS =====
    try:
        # Primeiro tenta importar do módulo (nome correto: GerenciadorSegredos)
        from src.modulos.gerenciador_segredos_real import GerenciadorSegredos
        # Renomeia para o nome esperado pelo sistema
        GerenciadorSegredosReal = GerenciadorSegredos
        logger.info("✅ GerenciadorSegredos importado de src.modulos (como GerenciadorSegredosReal)")
    except ImportError:
        try:
            # Fallback: tenta importar do caminho alternativo
            from src.camara.gerenciador_segredos_real import GerenciadorSegredos
            GerenciadorSegredosReal = GerenciadorSegredos
            logger.info("✅ GerenciadorSegredos importado de src.camara (como GerenciadorSegredosReal)")
        except ImportError:
            # Fallback final: criar uma versão simplificada
            logger.warning("⚠️ Nenhum GerenciadorSegredos encontrado. Usando versão simplificada.")
            
            class GerenciadorSegredosReal:
                """Versão simplificada para fallback"""
                def __init__(self):
                    self._segredos = {}
                    self._lock = __import__('threading').RLock()
                    self.logger = logging.getLogger("GerenciadorSegredosReal")
                    self.logger.info("✅ GerenciadorSegredosReal (fallback) inicializado")
                
                def obter_segredo(self, chave: str, padrao: any = None) -> any:
                    with self._lock:
                        return self._segredos.get(chave, padrao)
                
                def obter_api_key(self, servico: str) -> Optional[str]:
                    return None
                
                def definir_segredo(self, chave: str, valor: any) -> None:
                    with self._lock:
                        self._segredos[chave] = valor
                
                def listar_segredos(self):
                    with self._lock:
                        return self._segredos.copy()
    # =========================================================
    
    # Tentar ambos os locais para base_dados_arca
    try:
        from src.modulos.base_dados_arca import BaseDadosArca, obter_base_dados_arca
    except ImportError:
    ...
    raise RuntimeError(f"Componentes principais obrigatórios falharam: {e}")

try:
    from src.consulado.consulado_soberano import ConsuladoSoberano
    CONSULADO_DISPONIVEL = True
except Exception as e:
    logger.error(f"Erro ao importar ConsuladoSoberano: {e}")
    raise RuntimeError("ConsuladoSoberano obrigatório não disponível.")

try:
    from src.camara.gerador_almas import GeradorDeAlmas
    GERADOR_DE_ALMAS_DISPONIVEL = True
except Exception as e:
    logger.error(f"Erro ao importar GeradorDeAlmas: {e}")
    raise RuntimeError("GeradorDeAlmas obrigatório não disponível.")

try:
    from src.core.ciclo_de_vida import CicloDeVida
    CICLO_DE_VIDA_DISPONIVEL = True
except Exception as e:
    logger.error(f"Erro ao importar CicloDeVida: {e}")
    raise RuntimeError("CicloDeVida obrigatório não disponível.")
...
try:
    ...
    try:
        from src.config.config import get_config as get_config
        CONFIG_DISPONIVEL = True
    except ImportError as e:
        logger.error(f"Erro ao importar get_config: {e}")
        raise RuntimeError("get_config obrigatório não encontrado.")
...
def _obter_config() -> Dict[str, Any]:
    if CONFIG_DISPONIVEL and callable(get_config):
        try:
            ...
            raise RuntimeError(f"Erro ao obter configuração via get_config(): {e}")
    raise RuntimeError("configuração obrigatória não obtida.")

# ============================================================================
# FUNÇÃO PRINCIPAL DE INICIALIZAÇÃO (UNIFICADA)
# ============================================================================

def inicializar_sistema_completo() -> Dict[str, Any]:
    logger.info("[START] Iniciando inicialização do sistema completo...")
    ...
    # ===== USAR LlamaExeClient em vez de ParallelLLMEngine =====
    llm_engine = None
    try:
        from src.core.llama_exe_client import LlamaExeClient
        llm_engine = LlamaExeClient()
        ...
        logger.info(" -> LlamaExeClient inicializado (executável direto).")
    except Exception as e:
        logger.error(f"Erro ao inicializar LlamaExeClient: {e}")
        raise RuntimeError(f"LlamaExeClient obrigatório falhou: {e}")
    # =================================================================
    
    gerenciador_memoria = SistemaMemoriaHibrido(config=config)
    logger.info(" -> Sistema de Memória Hibrida inicializado.")
    
    cerebro = CerebroFamilia(memoria=gerenciador_memoria, config=config, llm_engine=llm_engine)
    logger.info(" -> Cérebro Família inicializado.")
    
    dispositivo_ai_ai = DispositivoAItoAI(config=config)
    logger.info(" -> Dispositivo AI↔AI inicializado.")
    ...
    try:
        ...
        raise RuntimeError(f"Base de Dados Arca obrigatória falhou: {e}")
    ...
    try:
        ...
        logger.info(" -> Analisador de intenções inicializado.")
    except Exception as e:
        logger.error(f"Erro ao inicializar AnalisadorIntencao: {e}")
        raise RuntimeError(f"Analisador de intenções obrigatório falhou: {e}")
    ...
    try:
        ...
        logger.info(" -> API de Encarnação inicializada.")
    except Exception as e:
        logger.error(f"Erro ao inicializar EncarnacaoAPI: {e}")
        raise RuntimeError(f"API de Encarnação obrigatória falhou: {e}")
    
    logger.info("Inicialização concluída. Retornando instâncias.")
    return {
        "config": config,
...
def main():
    ...
    p_show = sub.add_parser("show", help="Imprimir estado/flags para inspeção")
    args = ap.parse_args()
    if args.cmd == "remove-md":
        ...
        print("Inicialização retornou chaves:", ", ".join(sorted(inst.keys())))
        sys.exit(0)
    elif args.cmd == "show":
