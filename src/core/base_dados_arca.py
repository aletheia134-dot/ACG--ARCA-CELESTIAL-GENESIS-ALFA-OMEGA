...
Acesso seguro e somente leitura aos diários (SQLite) das AIs.Melhorias aplicadas:
 - Sanitização e whitelist de nomes de AIs (evita path traversal)
 ...
 - Conexão em modo URI (mode=ro) e tratamento defensivo de erros
 - Logs informativos e tratamento robusto de exceções
 - Métodos utilitários para listar AIs e caminho do DB
"""
...
class ObservadorArca:
    """
    Sistema de auditoria (somente leitura) que permite varreduras seguras dos diários das AIs.
    """
    ...
    def __init__(self, db_base_path: str = "Santuarios/Diarios", ais: Optional[List[str]] = None):
        ...
        # Validar diretório (não criamos nada aqui - somente leitura auditiva)
        if not self.db_base_path.exists():
            logger.error("ObservadorArca: caminho de diários não encontrado: %s", str(self.db_base_path))
    ...
    def _normalize_ai_name(self, ai_nome: str) -> Optional[str]:
        """Normaliza e valida o nome da AI contra whitelist (case-insensitive)."""
    ...
    def _db_path_for_ai(self, ai_nome: str) -> Optional[Path]:
        """Constrói o caminho para o DB do AI de forma segura (apenas para nomes permitidos)."""
        canonical = self._normalize_ai_name(ai_nome)
        if not canonical:
            logger.debug("Nome de AI não reconhecido: %s", ai_nome)
            return None
        db_path = self.db_base_path / f"diario_{canonical}.db"
        if not db_path.exists():
            logger.debug("Arquivo de diário não encontrado para %s: %s", canonical, db_path)
            return None
        return db_path

    def _conectar_ro(self, ai_nome: str) -> Optional[sqlite3.Connection]:
        """
        Abre conexão SQLite em modo somente leitura via URI.Retorna conexão com row_factory = sqlite3.Row ou None se não for possível.
        """
    ...
    def varredura_total(self, termo_busca: Optional[str] = None, limite_por_ai: int = 5) -> Dict[str, List[Dict[str, Any]]]:
        """
        Realiza busca transversal nos diários.Retorna dicionário: {ai: [rows...]}

        Observações de segurança:
         - termo_busca é limitado em comprimento e escapado para LIKE
         - limite_por_ai é limitado até MAX_RESULTS_PER_AI para evitar cargas grandes
        """
        ...
        for ai in self.ais:
            ...
            try:
                ...
                logger.exception("Erro inesperado ao auditar %s", ai)
            finally:
                try:
                    conn.close()
                except Exception:
    ...
    def verificar_integridade_memoria(self, ai_nome: str) -> Dict[str, Any]:
        """
        Analisa a saúde do banco de dados da AI:
         - total de registros
         - timestamp do registro mais antigo
         - presença da tabela transcricoes
        """
        db_path = self._db_path_for_ai(ai_nome)
        if not db_path:
    ...
    def obter_ultimo_pensamento(self, ai_nome: str) -> Optional[Dict[str, Any]]:
        """Recupera a última transcrição (mais recente) de uma AI específica."""
        ...
        try:
            ...
            logger.exception("Erro ao obter último pensamento de %s", ai_nome)
            return None
        finally:
            try:
                conn.close()
