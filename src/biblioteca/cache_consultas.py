...
def limpar_cache(config):
    """Limpa todo o cache e as estatísticas.

    Implementação: remove arquivos de cache no diretório configurado e
    reseta o arquivo de estatísticas.
    """
        ...
        for p in cache_dir.iterdir():
            if p.is_file() and not p.name.startswith(".keep"):
                try:
                    ...
                    logger.debug("Não foi possível remover cache %s", p, exc_info=True)
        try:
