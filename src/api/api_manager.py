...
# -*- coding: utf-8 -*-
"""
Gerencia interações com APIs externas delegando execução e segurança ao Consulado Soberano.
Implementação defensiva e real:
 - usa o Consulado quando disponível para todas as chamadas HTTP
 - valida ações via validador ético quando disponível
 - registra eventos relevantes na memória quando possível
 - caching local com TTL
 - implementações reais de provedores de clima e de consultas (NewsAPI, GitHub, Wikipedia, Reqres)
"""
...
class APIManager:
    def __init__(self, coracao_ref: Any, config_instance: Any = None):
        ...
        # Cache local com TTL (dicionário: {key: {"data": ..., "timestamp": ..., "ttl": ...}})
        ...
        if not hasattr(self.coracao, "secrets_manager") or not self.coracao.secrets_manager:
            self.logger.critical("[API_MANAGER] Secrets Manager não disponível no Coração.")
            raise ConfigError("Secrets Manager não disponível para APIManager.")

        # Carregar chaves (podem ser listas ou valores únicos)
    ...
    def _validar_acao_externa(self, acao: str, detalhes: str) -> bool:
        ...
        if callable(getattr(validador, "validar_acao", None)):
            try:
                ...
                if not aprovado:
                    self.logger.warning("[API_MANAGER] Validador ético bloqueou a ação: %s -> %s", acao, motivo)
                return bool(aprovado)
            except Exception:
                self.logger.exception("[API_MANAGER] Erro ao chamar ValidadorEtico; permitindo por fallback.")
                return True
        self.logger.warning("[API_MANAGER] Validador Ético não disponível — prosseguindo sem validação.")
        return True
    ...
    def _registrar_na_memoria(self, evento: str, categoria: str = "api_externa") -> None:
        ...
        try:
            if callable(getattr(gm, "registrar_evento_na_historia", None)):
                ...
                self.logger.debug("[API_MANAGER] Gerenciador de Memória não expõe registrar_evento_na_historia.")
        except Exception:
    ...
    async def _call_consulado_async(self, payload: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if not callable(fn):
            return {"status": "falha", "motivo": "Consulado não expõe solicitar_missao."}
        try:
    ...
    def _call_consulado(self, payload: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            if asyncio.iscoroutine(resultado):
                try:
                    if asyncio.get_event_loop().is_running():
                        self.logger.error("[API_MANAGER] Consulado retornou coroutine, loop já em execução.")
                        return {"status": "falha", "motivo": "Consulado assíncrono não suportado neste contexto."}
                    else:
                        resultado = asyncio.run(resultado)
                except Exception as e:
    ...
    def _chamar_via_consulado(self,
        ...
        if not hasattr(self.coracao, "consulado") or not self.coracao.consulado:
            self.logger.error("[API_MANAGER] Consulado Soberano não disponível.")
            return {"status": "falha", "motivo": "Consulado indisponível."}

        payload = {
            "acao": "requisicao_http",
            "descricao": descricao_extra or f"Requisição segura para {url}",
            "autor": "APIManager",
    ...
    async def obter_clima_atual(self, cidade: Optional[str] = None, pais: Optional[str] = None) -> Dict[str, Any]:
        ...
        cidade_final = cidade or self.OPENWEATHERMAP_CIDADE or "São Paulo"
        ...
        if not self._validar_acao_externa("OBTER_CLIMA", f"Obter clima para {cidade_final}, {pais_final}"):
            return {"sucesso": False, "mensagem": "Ação bloqueada por validador ético."}
        ...
        return {"sucesso": False, "mensagem": "Não foi possível obter clima de nenhum provedor."}
    ...
    async def _obter_clima_accuweather_completo_async(self, cidade: str, pais: str, api_key: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if not locations:
                return {"sucesso": False, "mensagem": "Cidade não encontrada no AccuWeather."}
    ...
    async def _obter_clima_tomorrow_io_completo_async(self, cidade: str, pais: str, api_key: str) -> Dict[str, Any]:
        ...
        # se não temos coords, tentamos geocoding via OpenWeatherMap (se chave disponível)
        if not coords:
        ...
        if not coords:
            self.logger.warning("[API_MANAGER] Coordenadas não encontradas para %s", cidade)
            return {"sucesso": False, "mensagem": f"Coordenadas não encontradas para {cidade}"}
    ...
    async def obter_noticias_newsapi(self, query: str = "IA", language: str = "pt") -> Dict[str, Any]:
        ...
        if not self._validar_acao_externa("OBTER_NOTICIAS", f"Obter notícias sobre {query}"):
            return {"sucesso": False, "mensagem": "Ação bloqueada por validador ético."}
        ...
        if not self.NEWSAPI_API_KEY or "SUA_CHAVE" in str(self.NEWSAPI_API_KEY).upper():
            ...
            return {"sucesso": False, "mensagem": "Chave da NewsAPI não configurada."}
        ...
        if res and res.get("status") == "executado":
            try:
                ...
                self._registrar_na_memoria(f"Notícias obtidas: {query}")
    ...
    # ----------------------
    # GitHub search (usa api_consultor se disponível, senão Consulado)
    # ----------------------
    async def consultar_github(self, query: str, sort: str = "updated") -> Dict[str, Any]:
        ...
        if not self._validar_acao_externa("CONSULTAR_GITHUB", f"Consultar repositórios sobre {query}"):
            return {"sucesso": False, "mensagem": "Bloqueado por validador ético."}
