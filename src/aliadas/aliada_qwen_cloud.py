...
class AliadaQwenCloud:
    ...
    def __init__(self, cfg: Optional[Dict[str, Any]] = None):
        self.cfg = cfg or {}
        self.endpoint = self.cfg.get("endpoint") or os.environ.get("QWEN_CLOUD_API_URL")
        self.api_key = self.cfg.get("api_key") or os.environ.get("QWEN_CLOUD_API_KEY")
        try:
            self.timeout = float(self.cfg.get("timeout_sec", os.environ.get("QWEN_CLOUD_TIMEOUT_SEC", DEFAULT_TIMEOUT)))
        except Exception:
        ...
        self._closed = False
        logger.info("AliadaQwenCloud initialized endpoint=%s retries=%s timeout=%s", self.endpoint, self.retries, self.timeout)
    ...
    def _extract_text(self, data: Any) -> Optional[str]:
        ...
        if isinstance(data, dict):
            for k in ("output", "result", "text", "generated_text", "prediction", "response"):
                v = data.get(k)
    ...
    def _call_api(self, payload: Dict[str, Any]) -> Dict[str, Any]:
        ...
        while attempt <= self.retries:
            ...
            try:
                ...
                last_exc = e
                logger.warning("AliadaQwenCloud timeout (attempt %d/%d): %s", attempt, self.retries + 1, e)
                if attempt > self.retries:
                    raise LLMTimeoutError("Qwen Cloud timeout") from e
            except RequestException as e:
                last_exc = e
                status = getattr(e.response, "status_code", None) if hasattr(e, "response") else None
                logger.warning("AliadaQwenCloud request error (attempt %d/%d) status=%s: %s", attempt, self.retries + 1, status, e)
                if attempt > self.retries:
                    raise LLMExecutionError(f"Qwen Cloud request failed: {e}") from e
            # backoff com jitter
        ...
        if last_exc:
            raise LLMExecutionError("Qwen Cloud request failed (exceeded retries)") from last_exc
        raise LLMExecutionError("Qwen Cloud request failed (unknown reason)")
    ...
    def processar(self, comando: str, contexto: Optional[Dict[str, Any]] = None) -> Tuple[bool, Optional[str], str]:
        ...
        try:
            ...
            logger.warning("Qwen Cloud timeout: %s", e)
            return False, None, "timeout"
        except LLMUnavailableError as e:
            logger.error("Qwen Cloud unavailable: %s", e)
            return False, None, "unavailable"
        except LLMExecutionError as e:
            logger.exception("Qwen Cloud execution error: %s", e)
            return False, None, "error"
        except Exception as e:
            logger.exception("Qwen Cloud unexpected error: %s", e)
            return False, None, "error"
    ...
    def shutdown(self) -> None:
        try:
            ...
            try:
                ...
                pass
            logger.info("AliadaQwenCloud.shutdown() called")
        except Exception:
            pass
