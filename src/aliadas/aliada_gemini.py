...
class AliadaGemini:
    """
    Cliente robusto para o serviço Gemini.
    ...
    def __init__(self, cfg: Optional[Dict[str, Any]] = None):
        ...
        if not self.endpoint:
            raise LLMUnavailableError(
                "AliadaGemini: endpoint não configurado (cfg['endpoint'] or GEMINI_API_URL)"
            )
    ...
    def _extract_text(self, data: Any) -> Optional[str]:
        ...
        Busca chaves comuns: output, result, text, generated_text, prediction, choices[0].text, data[0].text, etc.
        """
        if data is None:
            return None
    ...
    def processar(self, comando: str, contexto: Optional[Dict[str, Any]] = None) -> Tuple[bool, Optional[str], str]:
        ...
        """
        if self._closed:
            logger.error("AliadaGemini chamada após shutdown")
