...
try:
    ...
    logging.getLogger(__name__).warning("⚠️ MemoryFacade não disponível")
    MemoryFacade = None  # será tratado em runtime
...
# controla se a factory tentará criar coleções Chroma automaticamente
...
def _sanitize_nome_alma(raw: str) -> Optional[str]:
    ...
    if not nome:
        return None
    # permitir letras, números, underscore e hífen; evitar traversal/path chars
...
def _call_memory_facade_constructor(mf_class: Callable, nome: str, config: Any) -> Any:
    ...
    try:
        ...
        # fallback: tentar passar só nome
        return mf_class(nome)
    except TypeError:
        # último recurso: tentar qualquer combinação
        try:
...
class FacadeBundle(dict):
    """
    Dict-like retornado pela factory. Fornece utilitários:
    ...
    def get_or_create(self, nome: str, config: Any = None, criar_collections_chroma: Optional[bool] = None) -> Optional[Any]:
        ...
        try:
            ...
            if MemoryFacade is None:
                log.error("MemoryFacade não disponível no ambiente")
                return None
            ...
            self[nome_norm] = mf
            log.info("✅ MemoryFacade get_or_create criado para %s", nome_norm)
            return mf
        except Exception:
...
def inicializar_facades_memoria(
    ...
    Parâmetros:
      - sistema_memoria: instância de SistemaMemoriaHibrido (opcional)
      - config: objeto de configuração (opcional)
      - nomes_override: lista explícita de nomes de almas (opcional)
      - criar_collections_chroma: se True, tenta criar coleções Chroma (padrão via ENV)
      ...
      - timeout_por_alma: timeout em segundos por alma na criação paralela
    """
    log = logger or logging.getLogger("facade_factory")
    facades: Dict[str, Any] = {}

    if MemoryFacade is None:
        log.error("MemoryFacade não está disponível (import falhou). Retornando vazio.")
