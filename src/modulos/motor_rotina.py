# -*- coding: utf-8 -*-
"""
Motor de Monitoramento (antigo MotorDeRotina) - versão endurecida

Responsabilidades:
 - Monitorar saúde do sistema (CPU/RAM/Disco/processos)
 - Registrar histórico de diagnósticos com escrita atômica
 - Propor soluções ao "Pai" via coracao (defensivo)
...
Melhorias principais aplicadas:
 ...
 - Uso de threading.Event.wait para loops interrompíveis
 - Locks para proteger estado compartilhado
 - Proteção e timeouts ao enfileirar mensagens em coracao
 - Parsing robusto de logs (tail via deque) com dateutil fallback
 - Garantia de criação de diretórios antes de escrita
...
def _setup_config_getter(config_obj: Any):
    """Getter estrito — levanta se chave obrigatória estiver ausente."""
...
class MotorMonitor:
    ...
    def __init__(self, coracao_ref: Any, config: Any):
        ...
        try:
            ...
            self.logger.exception("Falha ao criar diretórios obrigatórios (continuando)")
        ...
        try:
            ...
            self.logger.debug("Não foi possível garantir diretórios de arquivos (continuando)")
    ...
    def _carregar_historico_diagnosticos(self) -> List[Dict[str, Any]]:
        ...
        if not caminho.exists():
            self.logger.debug("Arquivo de diagnósticos não existe, iniciando vazio")
            return []
        try:
            with caminho.open("r", encoding="utf-8") as fh:
                data = json.load(fh)
            self.logger.info("[MONITOR] %d diagnósticos carregados", len(data))
            return data
        except Exception:
            self.logger.exception("Falha ao carregar histórico de diagnósticos; iniciando vazio")
            return []
    ...
    def _salvar_historico_diagnosticos(self):
        ...
        try:
            ...
            self.logger.debug("[MONITOR] Histórico de diagnósticos salvo em %s", caminho)
        except Exception:
            self.logger.exception("Falha ao salvar histórico de diagnósticos")

    def _carregar_protocolo_habilidades(self) -> Dict[str, Any]:
        caminho = self.habilidades_path
        if not caminho.exists():
            self.logger.info("[MONITOR] Protocolo de habilidades não encontrado; inicializando vazio")
    ...
    def iniciar_monitoramento(self):
        with self._lock:
            if self._monitorando:
                self.logger.debug("Monitoramento já em execução")
                return
    ...
    def _loop_monitoramento(self):
        ...
        while not self._stop_event.is_set():
            try:
                ...
                # decide if idle
                try:
                    if self.pc_esta_ocioso(nivel=nivel_monitoramento):
                        self._realizar_diagnostico()
                    else:
                        self.logger.debug("[MONITOR] Sistema em uso ativo.Pulando diagnóstico.")
                except Exception:
    ...
    def _realizar_diagnostico(self):
        self.logger.debug("[MONITOR] Coletando métricas do sistema...")
        try:
        ...
        try:
            ...
            self.logger.exception("Erro ao coletar métricas psutil (fallback 0)")
            cpu = ram = uso_disco = 0.0
            processos_ativos = 0
        ...
        self.logger.info("[MONITOR] Diagnóstico: %s CPU=%.1f%% RAM=%.1f%%", estado, cpu, ram)
    ...
    def _analisar_logs_de_erro(self) -> List[str]:
        ...
        if not log_file.exists():
            self.logger.debug("Arquivo de log não encontrado para análise")
            return []
            ...
            if self.historico_diagnosticos:
                ...
                if last_ts:
                    ...
                    if parsed and (datetime.datetime.now() - parsed).total_seconds() < prazo_limite:
                        self.logger.debug("Proposta recente já enviada; pulando")
                        return

        self.logger.info("[MONITOR] Propondo solução ao Pai: %s", tipo_problema)
        # build prompts
        prompt_sistema = (
        ...
        try:
            ...
            if hasattr(self.coracao, "_enviar_para_cerebro"):
                try:
                    ...
                    self.logger.exception("Falha ao chamar cérebro (continuando)")
            if not proposta_str:
                self.logger.debug("Cérebro não retornou proposta; pulando enfileiramento")
                return
            try:
                proposta = json.loads(proposta_str)
            except Exception:
                self.logger.warning("Resposta do cérebro não é JSON válido: %s", (proposta_str[:200] + '...'))
                return
            required = {'nome_acao', 'descricao_acao', 'explicacao_proposito', 'comando_script'}
            if not required.issubset(proposta):
                self.logger.warning("Proposta inválida (campos faltando)")
                return
                ...
                if hasattr(self.coracao, "command_queue"):
                    try:
                        ...
                        try:
                            ...
                            self.logger.debug("Não foi possível enfileirar comando na command_queue")
            except Exception:
                self.logger.exception("Erro ao enfileirar proposta")
            self.logger.info("[MONITOR] Proposta enfileirada: %s", proposta.get("nome_acao"))
        except Exception:
            self.logger.exception("Erro ao propor solução (unexpected)")

    # --- utility: detect idle (platform-aware) ---
    def pc_esta_ocioso(self, nivel: str = "moderada") -> bool:
        """
        Determina se o sistema está ocioso com base em thresholds.Em Windows tenta usar GetLastInputInfo via ctypes; em outros usa psutil.
        """
        try:
            ...
            limiar_ociosidade = thresholds_map.get(nivel, thresholds_map["moderada"])
        except Exception:
            limiar_cpu = 20.0
            limiar_ram = 50.0
        ...
        if os.name == 'nt':
            try:
                ...
                self.logger.debug("Falha na detecção de idle via ctypes (Windows), fallback psutil")
