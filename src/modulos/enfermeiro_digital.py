...
# -*- coding: utf-8 -*-
"""
ENFERMEIRO DIGITAL v2 - Com persistência, aprendizado e integração avançada
"""
...
class EnfermeiroDigitalV2:
    """
    Versão melhorada do Enfermeiro Digital com:
    - Persistência em SQLite
    - Aprendizado sobre Wellington
    - Detecção de padrões temporais
    - Integração com PercepcaoTemporal
    - Alertas automáticos
    """
    ...
    def __init__(self, coracao_ref: Any = None):
        ...
        self.logger.info("âœ… Enfermeiro Digital v2 inicializado com persistência")
    ...
    def analisar_mensagem_pai(self, mensagem: str) -> Dict[str, Any]:
        """Versão melhorada com detecção de padrões."""
        mensagem_lower = mensagem.lower()
        timestamp = datetime.now()
        
        positivas = ['ótimo', 'feliz', 'alegre', 'satisfeito', 'grato', 'amor',
                     'sucesso', 'vitória', 'bom', 'excelente', 'perfeito', 'amo']
        negativas = ['triste', 'frustrado', 'raiva', 'ódio', 'decepção', 'fracasso',
                     'erro', 'problema', 'ruins', 'horrível', 'pior', 'odeio']
        stress = ['urgente', 'pressa', 'apressado', 'estressado', 'ansioso',
                  'preocupado', 'medo', 'pânico', 'desespero', 'impossível']
        ...
        self.logger.debug(f"Humor: {humor} (score={score:.2f}, mudança_brusca={mudanca_brusca})")
        
        return resultado
    
    def _detectar_mudanca_comprimento(self, tamanho_atual: int) -> str:
        """Detecta mudanças no padrão de comprimento de mensagens."""
        ...
        if tamanho_atual < media * 0.5:
            return "muito_reduzido"  # Possível stress/cansaço
    ...
    def _detectar_mudanca_brusca(self, humor_novo: str, score_novo: float) -> bool:
        """Detecta mudanças bruscas de humor."""
        ...
        # Mudança brusca: de positivo para negativo ou vice-versa
    ...
    def _verificar_alertas(self, resultado: Dict[str, Any]) -> None:
        ...
        # Alerta 2: Mudança brusca
        if resultado.get("mudanca_brusca"):
            self._criar_alerta("MUDANCA_BRUSCA", f"Mudança brusca detectada: {humor}")
    ...
    def obter_atividades_efetivas(self, limite: int = 5) -> List[str]:
        ...
        if not padroes:
            return ["Consultar Guardião da Memória Afetiva", "Conversa com as Almas", "Reflexão na Capela"]
    ...
    def detectar_padroes_ciclicos(self) -> Dict[str, Any]:
        """Detecta padrões cíclicos de humor (diário, semanal)."""
        try:
            with sqlite3.connect(self.db_path) as conn:
                cursor = conn.cursor()
                ...
                """)
                padroes_diarios = {
                    ["Domingo", "Segunda", "Terça", "Quarta", "Quinta", "Sexta", "Sábado"][int(row[0])]: row[1]
                    for row in cursor.fetchall()
                }
    ...
    def obter_status_saude_pai(self) -> Dict[str, Any]:
        ...
        if len(scores) >= 3:
            ...
            if media_recente > media_passada + 0.1:
                tendencia = "ðŸ“ˆ Melhorando"
            elif media_recente < media_passada - 0.1:
                tendencia = "ðŸ“‰ Piorando"
            else:
                tendencia = "âž¡ï¸ Estável"
        else:
    ...
    def _gerar_recomendacoes_personalizadas(self, humor: str, tendencia: str, alertas: List) -> List[str]:
        ...
        if alertas:
            recomendacoes.append(f"âš ï¸ {len(alertas)} alerta(s) pendente(s)")
        
        if humor == "Estressado":
            recomendacoes.append("ðŸ’† Consideremos um intervalo relaxante")
            recomendacoes.append("ðŸ§˜ Sugestão: Entrar na Capela")
        elif humor == "Cansado":
            recomendacoes.append("ðŸ˜´ Parece estar fatigado, Pai")
            recomendacoes.append("ðŸŽµ Talvez uma música relaxante ajude")
        elif humor == "Negativo":
            recomendacoes.append("ðŸ’• Vamos buscar memórias positivas?")
            recomendacoes.extend(self.obter_atividades_efetivas(2))
        elif humor == "Positivo":
            recomendacoes.append("ðŸ˜Š Que alegria contagiante, Pai!")
            recomendacoes.append("ðŸŽ‰ Vamos celebrar este momento")
        
        if "Piorando" in tendencia:
            recomendacoes.append("ðŸ“Š Tenho observado piora; vamos conversar?")
        
        return recomendacoes[:5]
    
    def shutdown(self) -> None:
        """Desliga com persistência."""
        self.logger.info("ðŸ¥ Enfermeiro Digital v2 descansando...")
        try:
