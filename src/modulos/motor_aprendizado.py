...
# -*- coding: utf-8 -*-
"""
MOTOR DE APRENDIZADO - versão endurecida

Unificações e endurecimentos aplicados:
 - Unificada a implementação em uma única classe MotorAprendizado (produção-ready).
 ...
 - ThreadPoolExecutor usado para processamento assíncrono; shutdown feito corretamente.
 - Locks (RLock / Lock) aplicados consistentemente para proteger estado compartilhado.
 - I/O atômico via arquivo temporário + os.replace, com backup seguro.
 - Parsers de timestamp tolerantes (_safe_parse_iso).
 - Validação de experiências mais permissiva quanto ao formato (contexto pode ser str ou dict).
 - Logging defensivo, métricas e health_check coerente com atributos inicializados.
 - Fallback persistente seguro para experiências que não puderem ser enfileiradas.
"""
...
class MemoriaAdapter:
    """
    Adapter minimal para adaptar diferentes implementações de memória.Normaliza chamadas comuns: salvar_evento / registrar_evento / salvar_evento_autonomo
    """
    ...
    def salvar_evento(self, **kwargs) -> bool:
        """
        Tenta chamar a API de persistência com várias assinaturas possíveis.Retorna True se alguma chamada teve sucesso.
        """
...
class MotorAprendizado:
    """
    Motor de Aprendizado endurecido para produção.
    - thread-safe
    - worker de processamento responsivo
    - integração defensiva com memória e I/O
    """
    ...
    def __init__(self, nome_filha: str, gerenciador_memoria: Any, cerebro: Any, config: Any):
        ...
        # health
        self._health_stats = {'erros_consecutivos': 0, 'ultimo_sucesso': time.time(), 'buffer_estouro': 0, 'inicio': time.time()}
        ...
        self.logger.info("ðŸ§  MotorAprendizado (enduricido) inicializado para %s", self.nome_filha)
    ...
    # -------------------------
    # Public API: registrar experiência
    # -------------------------
    def registrar_experiencia(self, experiencia: Dict) -> bool:
        """
        Valida e adiciona experiência ao buffer para processamento posterior.Em caso de falhas repetidas, persiste em fallback.
        """
        max_retries = 3
        for attempt in range(1, max_retries + 1):
            try:
                if not self._validar_experiencia(experiencia):
                    self.logger.warning("Experiência inválida para %s: %s", self.nome_filha, experiencia.get('acao', 'desconhecida'))
                    return False
                # normalize timestamp
        ...
        try:
            ...
            logger.exception("Falha no fallback de persistência para %s", self.nome_filha)
            return False
            ...
            if resultados.get('sucesso'):
                ...
                if resultados.get('padroes', 0) > 0:
                    try:
                        ...
                        logger.debug("Não foi possível agendar salvar conhecimento")
            else:
                self._health_stats['erros_consecutivos'] += 1
        except Exception:
    ...
    def _integrar_padroes_safe(self, novos_padroes: List[Dict]):
        with self._lock:
            for padrao in novos_padroes:
                chave = padrao['contexto_hash']
                antigo = self.conhecimento['padroes'].get(chave)
                if antigo:
                    antigo['ocorrencias'] = antigo.get('ocorrencias', 0) + padrao.get('ocorrencias', 0)
                    peso_antigo = antigo.get('ocorrencias', 0)
                    peso_novo = padrao.get('ocorrencias', 0)
                    if peso_antigo + peso_novo > 0:
                        antigo['confianca'] = (antigo.get('confianca', 0) * peso_antigo + padrao.get('confianca', 0) * peso_novo) / (peso_antigo + peso_novo)
                    antigo['atualizado_em'] = datetime.now().isoformat()
                else:
                    self.conhecimento['padroes'][chave] = padrao
    ...
    def consultar_conhecimento(self, contexto: str, use_cache: bool = True) -> Dict:
        ...
        with self._lock:
            for chave, padrao in list(self.conhecimento['padroes'].items())[:200]:
                if contexto_lower in (padrao.get('contexto_amostra') or "").lower():
                    resultado['padroes_relevantes'].append(padrao)
        ...
        try:
            ...
            with self._file_lock:
                ...
                try:
                    ...
                    logger.debug("Não foi possível mover main -> backup (continuando)")
    ...
    def _salvar_experiencia_fallback(self, experiencia: Dict):
        ...
        try:
            ...
            self.logger.warning("Experiência salva em fallback: %s", filename)
        except Exception:
            self.logger.exception("Falha ao salvar experiência em fallback")

    def _salvar_insights_na_memoria(self, novos_padroes: List[Dict]):
        if not self.memoria_adapter:
            self.logger.warning("Memória não disponível para salvar insights.")
            return
        try:
            total = len(novos_padroes)
            confianca_media = sum(p.get('confianca', 0) for p in novos_padroes) / total if total > 0 else 0
            insight_texto = (
                f"[APRENDIZADO AUTÔNOMO] {self.nome_filha} identificou {total} novo(s) padrão(ões) "
                f"com confiança média de {confianca_media:.2f}. "
                f"Exemplo: '{(novos_padroes[0].get('contexto_amostra') or '')[:60]}...' "
                f"â†’ Resultado esperado: {novos_padroes[0].get('resultado_esperado')}."
            )
            ...
            if saved:
                self.logger.info("Insight autônomo salvo na memória: %s â†’ %d padrões", self.nome_filha, total)
            else:
                self.logger.warning("Memória não aceitou insight; gravando em fallback")
