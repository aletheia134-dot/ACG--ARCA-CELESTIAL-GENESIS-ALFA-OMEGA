...
# -*- coding: utf-8 -*-
"""
src/ui/core.py - Núcleo de Integração da Interface do Usuário (UI Core)

Este módulo centraliza a integração da UI com o CoracaoOrquestrador.
Gerencia inicialização, comunicação assíncrona via filas, estados globais
e coordenação entre componentes (tray, janela principal, avatares).

Suporte a 6 AIs com avatares específicos para emoções detalhadas.
"""
...
class UICore:
    """
    Núcleo da UI: Integra tray, janela principal e avatares com o CoracaoOrquestrador.
    ...
    # Lista completa de emoções (baseada na sua especificação)
    EMOCOES_SUPORTADAS = [
    ...
    def __init__(self, ui_queue: queue.Queue, config: Dict[str, Any], coracao_ref=None):
        ...
        self.command_queue: queue.Queue = queue.Queue()  # Para comandos da UI ao coração
        self.response_queue: queue.Queue = queue.Queue()  # Para respostas do coração à UI
        ...
        logger.info("✅ UICore inicializado")
    ...
    def _carregar_avatares(self) -> None:
        ...
        logger.info(f"✅ Avatares carregados para {len(self.estado.avatares_carregados)} AIs")
    ...
    def _fallback_inicializacao(self) -> None:
        """Fallback se componentes falharem (ex.: avatares genéricos)."""
    ...
    def iniciar(self) -> None:
        ...
        logger.info("✅ Loop de processamento UI iniciado")
    ...
    def _processar_resposta(self, resposta: Dict[str, Any]) -> None:
        """Processa resposta do coração (ex.: atualizar UI)."""
    ...
    def _atualizar_avatares(self) -> None:
        """Atualiza avatares com estados atuais do coração."""
        if not self.coracao_ref:
            return
        for ai, frame in self.frames_avatares.items():
            # Obter emoção atual do motor de expressão (se disponível)
    ...
    def enviar_comando(self, comando: Dict[str, Any]) -> None:
        """Método público para enviar comandos à UI (ex.: do coração)."""
