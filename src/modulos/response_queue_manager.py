...
# -*- coding: utf-8 -*-
"""
RESPONSE QUEUE MANAGER v2 - Com priorização, deduplicação, batching, rate limiting, métricas
"""
...
class ResponseQueueManagerV2:
    ...
    - Fila com 3 níveis de prioridade (CRÍTICA, ALTA, NORMAL)
    ...
    def __init__(self, maxsize: int = 1000, rate_limit: int = 10, persistir_criticas: bool = True):
        ...
        self.logger.info(f"âœ… ResponseQueueManager v2 inicializado (rate_limit={rate_limit}/s)")
    ...
    def put(self, payload: Dict[str, Any], timeout: Optional[float] = None, 
        ...
        Adiciona mensagem com prioridade.Args:
            payload: dicionário com mensagem
            timeout: tempo máximo de espera
            priority: 0=crítica, 1=alta, 2=normal
            critical: força persistência mesmo se fila cheia
        
        Retorna:
            True se enfileirada, False caso contrário
        """
        if not isinstance(payload, dict):
            self.logger.warning("Payload inválido (não é dict)")
            return False
        ...
        if hash_payload in self.hashes_recentes:
            self.logger.debug(f"Duplicação detectada; descartando: {payload.get('tipo_resp')}")
            return False
    ...
    def shutdown(self) -> None:
        ...
        self.logger.info("ðŸš€ ResponseQueueManager v2 desligando...")
        
        # Flush batches pendentes
        self._flush_batches()
        
        # Métricas finais
        self.logger.info(f"Métricas finais: {self.obter_metricas()}")
        
        self.clear()
        self.logger.info("âœ… Desligamento completo")
