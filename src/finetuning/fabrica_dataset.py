"""
Fábrica de Dataset Automática para LoRA
========================================
Gera datasets de image captioning prontos para fine-tuning LoRA.

CORREÇÍO PRINCIPAL:
 - 'captiv' NÍO É UM PACOTE REAL. O arquivo original falhava imediatamente no import.
 - Substituído por BLIP-2 via HuggingFace Transformers (pacote real, amplamente usado).
 - Adicionado modo alternativo com BLIP base (mais leve, sem necessidade de GPU potente).
 - Adicionado argparse para rodar sem input() interativo (automatizável).
 - Adicionado tratamento real de erros no carregamento do modelo.
 - GIF e BMP excluídos: BLIP/BLIP-2 não suportam bem esses formatos.
...
try:
    ...
    logger.warning("PyTorch não encontrado. O carregamento do modelo vai falhar.")
...
def carregar_modelo(modo: str):
    ...
    try:
        ...
        print("ERRO: transformers não instalado. Execute: pip install transformers")
        raise SystemExit(1)
    ...
    if modo == "blip2":
        ...
        if device == "cpu":
            logger.warning("BLIP-2 em CPU será extremamente lento. Considere usar --modo blip para CPU.")
        try:
            ...
            logger.error("Verifique sua conexão, espaço em disco e VRAM disponível.")
            raise
    else:
...
def descrever_imagem(processor, model, device, img_path: Path, modo: str) -> str:
    """
    Gera descrição de uma imagem usando o modelo carregado.
    Retorna string vazia em caso de falha (não lança exceção — o loop de batch não para).
    """
    try:
        import torch
...
def gerar_perguntas(img_name: str) -> list:
    """
    Gera variações de perguntas para uma imagem.
    Mantido do original — esta parte estava correta.
    """
    return [
        f"Descreva a imagem {img_name}",
        f"O que tem na foto {img_name}?",
        f"Explique o conteúdo da imagem {img_name}",
        f"Como é a cena mostrada em {img_name}?",
...
def main():
    parser = argparse.ArgumentParser(description="Fábrica de Dataset para LoRA via Image Captioning")
    ...
    parser.add_argument("--split", type=float, default=0.8,
                        help="Proporção de treino (0.0 a 1.0, default: 0.8)")
    ...
    # BLIP/BLIP-2 funcionam bem com JPG e PNG. GIF e BMP excluídos intencionalmente.
    ...
    for img_path in tqdm(imagens, desc="Gerando descrições"):
        descricao = descrever_imagem(processor, model, device, img_path, args.modo)

        if not descricao:
            erros.append(f"{img_path.name}: falha na geração de descrição")
            continue

        # Salva descrição individual
        txt_path = pasta_txt / f"{img_path.stem}.txt"
        try:
            txt_path.write_text(descricao, encoding="utf-8")
        except Exception as e:
            logger.warning("Não foi possível salvar TXT para %s: %s", img_path.name, e)

        # Gera múltiplas entradas por imagem
    ...
    if validacao:
        ...
        logger.warning("Dataset muito pequeno para criar validação separada.")
    ...
    print(f"Itens para validação: {len(validacao)}")
