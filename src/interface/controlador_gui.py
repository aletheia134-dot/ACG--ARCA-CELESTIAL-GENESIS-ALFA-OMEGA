...
Abstrao segura para controle de mouse/teclado e screenshots, pensada para:
 ...
 - evitar combinaes perigosas e impedir execução de ações críticas

Design decisions:
 - imports de pyautogui / PIL so feitos de forma lazy dentro de __init__ e métodos,
   para permitir import do módulo em ambientes sem GUI.
 - Se GUI no estiver disponível, os métodos devolvem False/None e logam aviso.
 - Apenas comandos simples so permitidos (ex.: press de uma nica tecla).
...
class GUIUnavailableError(RuntimeError):
    """Indica que APIs de controle GUI no esto disponíveis no runtime."""


class ControladorGUI:
    """
    Controlador seguro para automao de UI.Args:
    ...
    def _ensure_gui_imports(self) -> None:
        ...
        try:
            ...
            self._PIL_Image = None
            self._gui_available = False
            logger.warning("pyautogui no disponível ou environment sem display; ControladorGUI entra em modo degradado.")

    def is_gui_available(self) -> bool:
        """Retorna True se as APIs GUI esto disponíveis no ambiente."""
    ...
    def _validar_velocidade_teclado(self, texto: str) -> bool:
        """Evita digitao em velocidade superior ação limite."""
        now = time.time()
        ...
        if velocidade > self._limite_velocidade_teclado:
            logger.warning("Digitao bloqueada por velocidade: %.1f chars/s (limite %.1f)", velocidade, self._limite_velocidade_teclado)
            return False
        self._last_keyboard_time = now
        return True
    ...
    def mover_mouse_para(self, x: int, y: int, duracao: float = 0.5) -> bool:
        ...
        """
        if not self.is_gui_available():
            logger.warning("Mover mouse solicitado mas GUI no disponível.")
            return False
        with self._lock_gui:
            if not isinstance(x, int) or not isinstance(y, int):
                logger.warning("Coordenadas invlidas para mover_mouse_para")
                return False
            if not self._validar_posicao_mouse(x, y):
                logger.warning("Posio fora da rea permitida: (%s,%s)", x, y)
                return False
    ...
    def clicar_em(self, x: int, y: int, botao: str = "left", duracao_movimento: float = 0.2) -> bool:
        """
        Move e clica em (x,y). válida posio, velocidade e evita spam de cliques.
        """
        if not self.is_gui_available():
            logger.warning("Clicar solicitado mas GUI no disponível.")
            return False
        with self._lock_gui:
            now = time.time()
            if now - self._last_click_time < 0.05:
                logger.warning("Clique bloqueado: muito rpido entre cliques.")
                return False
    ...
    def digitar_texto(self, texto: str, intervalo_entre_caracteres: float = 0.05) -> bool:
        ...
        if not self.is_gui_available():
            logger.warning("Digitar texto solicitado mas GUI no disponível.")
            return False
        if not isinstance(texto, str) or texto == "":
            logger.warning("digitar_texto chamado com texto invlido.")
            return False
        with self._lock_gui:
    ...
    def pressionar_tecla(self, tecla: str) -> bool:
        """
        Pressiona uma nica tecla permitida.No permite combinaes (hotkeys) aqui.
        """
        if not self.is_gui_available():
            logger.warning("Pressionar tecla solicitado mas GUI no disponível.")
            return False
        if not isinstance(tecla, str) or tecla.strip() == "":
            logger.warning("Tecla invlida solicitada.")
            return False
        ...
        for patt in self.DANGEROUS_KEY_PATTERNS:
            if patt.search(tecla_norm):
                logger.warning("Tecla proibida por padrão de segurana: %s", tecla)
                return False

        if "+" in tecla_norm or " " in tecla_norm:
            # do not allow hotkey combos via this method
            logger.warning("Combinaes de teclas no permitidas por pressionar_tecla: %s", tecla)
            return False

        if tecla_norm not in self._allowed_keys:
            logger.warning("Tecla no permitida: %s", tecla)
            return False
    ...
    def obter_posicao_mouse(self) -> Optional[Tuple[int, int]]:
        """Retorna posio do mouse ou None se indisponível."""
        if not self.is_gui_available():
            logger.debug("solicitação de posio do mouse mas GUI indisponível.")
            return None
        try:
    ...
    def obter_screenshot(self, caminho_saida: Optional[Path] = None, regiao: Optional[Tuple[int, int, int, int]] = None, max_pixels: Optional[int] = None) -> Optional[bytes]:
        ...
        """
        if not self.is_gui_available():
            logger.warning("solicitação de screenshot mas GUI no disponível.")
            return None
        with self._lock_gui:
            try:
                ...
                if caminho_saida:
                    try:
                        ...
                        if allowed_base not in target.parents and allowed_base != target.parent:
                            logger.warning("Caminho de screenshot no permitido: %s", caminho_saida)
                            caminho_saida = None
                        else:
    ...
    def obter_historico_acoes_gui(self) -> List[Dict[str, Any]]:
        """Retorna cpia do histórico de auditoria em memória."""
        with self._lock_gui:
    ...
    # -------------------------
    # Utilities para testes e administrao
    # -------------------------
