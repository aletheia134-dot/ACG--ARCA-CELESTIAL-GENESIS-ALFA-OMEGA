...
class PainelCameraSom(PainelBase):
    ...
    def _toggle_camera(self):
        """Ativa ou desativa câmera chamando o servidor media real (porta 5001)."""
        ...
        if self.camera_active:
            try:
                ...
                if resp.status_code == 200:
                    ...
                    self.camera_active = False  # reverte se falhou
...
class PainelSentimentos(PainelBase):
    def __init__(self, parent, coracao, ui_ref):
        super().__init__(parent, coracao, ui_ref)
        # guardiao_memoria_afetiva NÃO existe no coração — usar os módulos reais:
        # estados_emocionais, motores_curiosidade, validador_emocoes
    ...
    def _iniciar(self):
        """Inicia monitoramento emocional usando o coração real."""
        if self.coracao and hasattr(self.coracao, "avaliar_estados_internas_todas_almas"):
            try:
    ...
    def _relatorio(self):
        """Gera relatório afetivo usando os estados emocionais reais do coração."""
        if not self.coracao:
    ...
    def _sugestao(self):
        """Força geração de desejo/sugestão usando o motor de curiosidade real."""
...
class PainelFinetuning(PainelBase):
    ...
    def _ver_dataset(self):
        ...
        if not p.exists():
            ...
            return
        # Analisar dataset diretamente (sem dependência de DetectorUniversal)
        try:
...
class JanelaPrincipalArca(ctk.CTk):
    ...
    def _loop_respostas(self):
        ...
        while not self._response_thread_stop.is_set():
            try:
                # VERIFICAÇÃO CRÍTICA: garantir que response_queue é uma fila válida
    ...
    def _limpar_resposta_llm(texto: str) -> str:
        """Remove tokens de modelo/LoRA que vazaram na saída antes de exibir."""
        import re
        # Tokens de template e LoRA customizados (fine-tuning EVA/YUNA/etc)
        tokens_banidos = [
            "<|im_start|>", "<|im_end|>", "<|endoftext|>",
            "<|user|>", "<|system|>", "<|assistant|>", "<|human|>", "<|bot|>",
            "</s>", "<s>", "<|end|>", "[INST]", "[/INST]", "<<SYS>>", "<</SYS>>",
            # Tokens LoRA do fine-tuning
            ...
            "<user|>",  # versão sem | inicial (bug de tokenização)
        ]
        for tok in tokens_banidos:
            texto = texto.replace(tok, "")

        # Se ainda há algum <|...| ou <|...> desconhecido, corta antes dele
        m = re.search(r"<\|[a-zA-ZÀ-ú_àáâãéêíóôõúüç]+", texto)
        if m:
            texto = texto[:m.start()].strip()

        # Remove linhas que são só pipes, underscores ou sinais (artefatos)
        texto = re.sub(r"(?m)^\s*[|_=\-]+\s*$", "", texto)

        # Detecta e corta loops de repetição (ex: "um dia, um dia, um dia...")
        ...
        # Remove linhas em branco excessivas e espaços sobrando
        texto = re.sub(r"\n{3,}", "\n\n", texto)
        return texto.strip()
    ...
    def _handle_response(self, dados: Dict[str, Any]):
        try:
            ...
            # Limpa tokens de modelo/LoRA ANTES de exibir
...
# --------------------------------------------------------------------
#  FUNÇÃO DE ENTRADA USADA PELO main.py
# --------------------------------------------------------------------
def criar_interface(coracao_ref=None, ui_queue=None):
    """
    ...
    # Registrar command_queue no coracao para o processador de chat funcionar
