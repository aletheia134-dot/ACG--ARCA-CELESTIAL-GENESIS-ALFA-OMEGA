...
Interface grfica para consulta  biblioteca teolgica.
"""
import logging
...
# --- IMPORTS LOCAIS ---
from src.interface.interface_biblioteca import BibliotecaJWOtimizada
...
class JanelaBiblioteca(ctk.CTkFrame):
    """
    Frame da UI para consulta  biblioteca teolgica.
    ...
    def __init__(
        ...
        # Exemplos de injeo de dependncias da UI principal
    ...
    def setup_ui(self):
        ...
        self.grid_rowconfigure(2, weight=1)  # A rea de resultados ocupa o espao restante

        # --- 1.Ttulo ---
        lbl_titulo = ctk.CTkLabel(self, text=" Consulta  Biblioteca Teolgica",
        ...
        self.entry_pergunta = ctk.CTkEntry(frame_entrada, placeholder_text="Digite sua pergunta (ex: Joo 3:16 ou o que a Bblia diz sobre f?)")
        ...
        # Boto de Consulta
        btn_consultar = ctk.CTkButton(frame_entrada, text=" Consultar", command=self.on_consultar_click)
        ...
        self.lbl_resultados_placeholder = ctk.CTkLabel(self.frame_resultados_scroll, text="Resultados aparecero aqui aps a consulta.", justify="left")
        self.lbl_resultados_placeholder.pack(fill="both", expand=True, padx=5, pady=5)

        # --- 4.Frame de Controles (Exportar, Estatsticas) ---
        ...
        btn_estatisticas = ctk.CTkButton(frame_controles, text="Estatsticas", command=self.on_estatisticas_click)
        btn_estatisticas.grid(row=0, column=2, padx=5, pady=5, sticky="e")

    def on_consultar_click(self):
        """Manipulador do clique no boto 'Consultar'."""
        ...
        lbl_loading = ctk.CTkLabel(self.frame_resultados_scroll, text=" Buscando...", font=ctk.CTkFont(slant="italic"))
        ...
        try:
            ...
            # armazenar ltimo resultado para exportao
    ...
    def _atualizar_resultados_ui(self, resultado: Dict[str, Any]):
        """Atualiza a rea de resultados com os dados da consulta."""
        ...
        lbl_tempo = ctk.CTkLabel(self.frame_resultados_scroll, text=f" Tempo total: {tempo_total} ms", font=ctk.CTkFont(weight="bold"))
        lbl_tempo.pack(anchor="w", padx=5, pady=(0, 5))

        # Exibir anlise de contexto (se houver)
        ...
        if analise:
            ...
            lbl_analise = ctk.CTkLabel(self.frame_resultados_scroll, text=f" Anlise: Tipo='{tipo_busca}', Palavras-chave='{palavras_chave}'", font=ctk.CTkFont(size=12))
        ...
        if not resultados_lista:
            lbl_sem_result = ctk.CTkLabel(self.frame_resultados_scroll, text="[ERRO] Nenhum resultado encontrado.", text_color="gray")
        ...
        for i, (res, prev) in enumerate(zip(resultados_lista, previews_lista)):
            ...
            lbl_header = ctk.CTkLabel(frame_result_item, text=f" {i+1}. Fonte: {fonte} (Sim.: {sim_str})", font=ctk.CTkFont(weight="bold"))
            lbl_header.pack(anchor="w", padx=5, pady=(2, 0))

            # Contedo/Preview
            preview_text = prev.get('preview', res.get('conteudo', 'N/A'))  # Usa preview gerado, se disponível, seno o contedo bruto
    ...
    def on_exportar_click(self, formato: str):
        """Manipulador do clique em botes de exportao."""
        if hasattr(self, '_ultimo_resultado') and self._ultimo_resultado:
            ...
            messagebox.showwarning("Aviso", "Nenhum resultado recente para exportar. Faa uma consulta primeiro.")

    def on_estatisticas_click(self):
        """Manipulador do clique no boto 'Estatsticas'."""
        try:
            estatisticas = self.biblioteca_principal.obter_estatisticas()
            # Exibir estatsticas em uma messagebox ou em uma nova janela/aba
            stats_text = "\n".join([f"{k}: {v}" for k, v in estatisticas.items()])
            messagebox.showinfo("Estatsticas da Biblioteca", stats_text)
        except Exception as e:
            logger.error(f"Erro ao obter estatsticas via UI: {e}")
            messagebox.showerror("Erro", f"Erro ao obter estatsticas: {e}")
