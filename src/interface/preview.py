...
Componente para gerao de trechos destacados dos resultados de busca.
"""
...
class PreviewInteligente:
    ...
    def __init__(self, tamanho_max_palavras: int = 50, contexto_antes_depois: int = 10):
        ...
        logger.info(
            " Preview Inteligente inicializado (tamanho_max=%d, contexto=%d).",
    ...
    def gerar_previews(
        ...
        Retorna lista de dicionrios com chaves:
            - conteudo_original
            - preview
            - fonte
            - id (se disponível)
        ...
        for res in resultados:
            ...
            if not conteudo:
                previews.append(
                    {
                        "conteudo_original": "",
                        "preview": "[Sem contedo para gerar preview]",
                        "fonte": fonte,
                        "id": id_resultado,
                    }
                )
    ...
    def _gerar_preview_individual(self, conteudo: str, consulta_norm: str, estrategia: str) -> str:
        """Gera um preview para um único contedo dado a estratgia escolhida."""
        conteudo_norm = _normalize_text(conteudo)
        if not conteudo_norm or not consulta_norm:
            # fallback: incio do documento
    ...
    def _extrair_palavras_chave_simples(self, texto_norm: str) -> List[str]:
        """Extrai tokens do texto normalizado (remoo de stopwords simples)."""
        ...
        # preservar ordem de apario, sem duplicatas
        seen = set()
        ordered = []
        for k in keywords:
            if k not in seen:
    ...
    def _preview_por_keywords(self, conteudo: str, conteudo_norm: str, consulta_norm: str) -> str:
        """Gera preview centrado na primeira ocorrncia de palavras-chave encontradas."""
        palavras_chave = self._extrair_palavras_chave_simples(consulta_norm)
        if not palavras_chave:
            return self._trecho_inicio(conteudo)

        # trabalhar com tokens do contedo para localizar índices por token
        conteudo_tokens = re.findall(r"\w+|\S", conteudo)  # mantm palavras e sinais como separadores
    ...
    def _preview_por_sequence_match(self, conteudo: str, conteudo_norm: str, consulta_norm: str) -> str:
        """Gera preview baseado no maior trecho correspondente entre consulta e contedo."""
        ...
        # match.b  posio em caracteres no conteudo_norm (approx). Mapear para índice de palavras
