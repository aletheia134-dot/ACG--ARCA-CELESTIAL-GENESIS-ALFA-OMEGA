...
"""
ðŸªŸ Painel de Controle Estilo Windows 11 - VERSÍO 2.2
Central de todas as ferramentas.

CORREÇÕES v2.2:
  - Thread-safety no monitoramento (janela.after em vez de configurar widget direto da thread)
  - GPUtil com fallback quando não instalado
  - painel_controle não depende de psutil.disk_usage('C:/') no Linux
  - monitorar() não mistura time.sleep com janela.after inutilmente
  - clarear_cor() implementado de verdade
  - Barra de status com cor dinâmica (verde/amarelo/vermelho)
"""
...
try:
    ...
    PSUTIL_OK = False
    print("âš ï¸ psutil não instalado: pip install psutil")
...
# Mapa completo de ferramentas (nome â†’ arquivo)
# Atualiza esta lista ao adicionar novas ferramentas
# â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
MAPA_FERRAMENTAS = {
    "01_IMAGEM": [
        ("ðŸŽ¨ Foto â†’ Anime",          "Ferramenta_Anime.py"),
        ("ðŸ‘´ Envelhecer Rosto",       "Ferramenta_Envelhecer.py"),
        ("ðŸ” OCR - Extrair Texto",    "Ferramenta_OCR.py"),
        ("ðŸ–¼ï¸ Remover Fundo",          "Ferramenta_RemoverFundo.py"),
        ("ðŸ‘¤ Clonagem Rosto 3D",      "Ferramenta_ClonagemRosto3D.py"),
        ("ðŸŽ¨ Anime Video",            "Ferramenta_AnimeVideo.py"),
    ],
    "02_AUDIO": [
        ("ðŸŽ¤ Transcrição (Whisper)",  "Ferramenta_Transcricao.py"),
        ("ðŸ”Š Texto para Voz",         "Ferramenta_TextoParaVoz.py"),
        ("ðŸ”‡ Remover Ruído",          "Ferramenta_RemoverRuido.py"),
        ("ðŸŽµ Separar Voz",            "Ferramenta_SepararVoz.py"),
        ("ðŸ”„ Converter Íudio",        "Ferramenta_ConverterAudio.py"),
        ("ðŸŽ­ Clonar Voz",             "Ferramenta_ClonarVoz.py"),
    ],
    "03_VIDEO": [
        ("ðŸŽžï¸ Extrair Frames",         "Ferramenta_ExtrairFrames.py"),
        ("ðŸ“ Legendas Automáticas",   "Ferramenta_Legendas.py"),
        ("ðŸŽµ Extrair Íudio",          "Ferramenta_ExtrairAudio.py"),
        ("âœ‚ï¸ Cortar Vídeo",           "Ferramenta_CortarVideo.py"),
        ("ðŸŽ¬ Juntar Vídeos",          "Ferramenta_JuntarVideos.py"),
    ],
    "04_CAMERA": [
        ("ðŸ“· Webcam",                 "Ferramenta_Webcam.py"),
        ("ðŸŽ¨ Webcam Anime",           "Ferramenta_WebcamAnime.py"),
        ("ðŸ‘¤ Detectar Faces",         "Ferramenta_DetectarFaces.py"),
        ("ðŸ” Detectar Objetos",       "Ferramenta_DetectarObjetos.py"),
        ("ðŸƒ Pose Detection",         "Ferramenta_PoseDetection.py"),
    ],
    "05_DOCUMENTOS": [
        ("ðŸ“„ PDF para Texto",         "Ferramenta_PDFparaTexto.py"),
        ("ðŸ“ Word para Texto",        "Ferramenta_WordparaTexto.py"),
        ("ðŸ“Š Excel para CSV",         "Ferramenta_ExcelparaCSV.py"),
    ],
    "06_UTILIDADES": [
        ("ðŸ“¥ Downloader",             "Ferramenta_Downloader.py"),
        ("ðŸ“ Organizador",            "Ferramenta_Organizador.py"),
        ("ðŸ—œï¸ Compressor",             "Ferramenta_Compressor.py"),
    ],
}

CATEGORIAS = [
    {"id": "01_IMAGEM",      "nome": "Imagem",      "icone": "ðŸŽ¨", "cor": "#FFB900"},
    {"id": "02_AUDIO",       "nome": "Íudio",        "icone": "ðŸŽµ", "cor": "#0078D4"},
    {"id": "03_VIDEO",       "nome": "Vídeo",        "icone": "ðŸŽ¬", "cor": "#107C10"},
    {"id": "04_CAMERA",      "nome": "Câmera",       "icone": "ðŸ“·", "cor": "#D83B01"},
    {"id": "05_DOCUMENTOS",  "nome": "Documentos",   "icone": "ðŸ“„", "cor": "#881798"},
    {"id": "06_UTILIDADES",  "nome": "Utilidades",   "icone": "ðŸ› ï¸", "cor": "#767676"},
]
...
class PainelControleWindows11:
    def __init__(self):
        self.janela = ctk.CTk()
        self.janela.title("ðŸªŸ Central de Ferramentas IA")
    ...
    def _setup_interface(self):
        ...
        ctk.CTkLabel(logo_frame, text="ðŸªŸ", font=("Segoe UI", 24), text_color="#0078D4").pack(side="left", padx=(0,8))
        ...
        # Botão Início
        self._criar_btn_menu("ðŸ ", "Início", command=self._mostrar_inicio)
        ...
        ctk.CTkFrame(self.barra_lateral, height=1, fg_color="#404040").pack(fill="x", padx=15, pady=8)
        self._criar_btn_menu("âš™ï¸", "Configurações", command=self._mostrar_configuracoes)
        self._criar_btn_menu("â“", "Sobre",          command=self._mostrar_sobre)
        ...
        self.lbl_status_ponto = ctk.CTkLabel(status_esq, text="â—", font=("Segoe UI", 12), text_color="#107C10")
    ...
    def _mostrar_inicio(self):
        self._limpar_area()
        self._set_status("Início", "#107C10")

        scroll = ctk.CTkScrollableFrame(self.area, fg_color="transparent")
        scroll.pack(fill="both", expand=True, padx=20, pady=20)

        ctk.CTkLabel(scroll, text="ðŸ  Início", font=("Segoe UI", 28, "bold"), text_color="#0078D4").pack(anchor="w", pady=(0, 20))
    ...
    def _mostrar_categoria(self, cat: dict):
        ...
        for nome, arquivo in ferramentas:
            ...
            ctk.CTkButton(
                btn_frame, text="â–¶ Abrir", width=100,
            ...
            if not existe:
                ctk.CTkLabel(btn_frame, text="âŒ arquivo não encontrado",
                             text_color="#E81123", font=("Segoe UI", 11)).pack(side="left", padx=5)

    def _mostrar_configuracoes(self):
        self._limpar_area()
        self._set_status("âš™ï¸ Configurações", "#767676")

        scroll = ctk.CTkScrollableFrame(self.area, fg_color="transparent")
        scroll.pack(fill="both", expand=True, padx=20, pady=20)

        ctk.CTkLabel(scroll, text="âš™ï¸ Configurações",
        ...
        ctk.CTkButton(linha_pasta, text="ðŸ“ Alterar", width=100,
        ...
        ctk.CTkButton(scroll, text="ðŸ’¾ Salvar configurações", width=200,
                      fg_color="#0078D4", command=self._salvar_configuracoes).pack(pady=20)

    def _mostrar_sobre(self):
        self._limpar_area()
        self._set_status("â“ Sobre", "#767676")

        scroll = ctk.CTkScrollableFrame(self.area, fg_color="transparent")
        scroll.pack(fill="both", expand=True, padx=20, pady=20)

        ctk.CTkLabel(scroll, text="â“ Sobre", font=("Segoe UI", 28, "bold"), text_color="#0078D4").pack(anchor="w")

        card = ctk.CTkFrame(scroll, fg_color="#2D2D2D", corner_radius=12)
        card.pack(fill="both", expand=True, pady=10)

        ctk.CTkLabel(card, text="ðŸªŸ", font=("Segoe UI", 72), text_color="#0078D4").pack(pady=(30, 5))
        ctk.CTkLabel(card, text="Central de Ferramentas IA", font=("Segoe UI", 22, "bold")).pack()
        ctk.CTkLabel(card, text=f"Versão 2.2 • {self.total_ferramentas} Ferramentas",
                     font=("Segoe UI", 14), text_color="#AAAAAA").pack(pady=5)
        ctk.CTkLabel(card, text=f"GPU: {'âœ… Ativa' if USAR_GPU else 'âŒ Inativa (CPU)'}",
    ...
    def _iniciar_monitoramento(self):
        """Lança thread de monitoramento. Thread NÍO toca em widgets tkinter diretamente."""

        def _loop():
            while self.monitorando:
                dados = self._coletar_metricas()
                # Envia atualização para a thread principal via after()
                try:
                    self.janela.after(0, lambda d=dados: self._aplicar_metricas(d))
                except Exception:
                    break  # Janela foi destruída
                time.sleep(2)
    ...
    def _coletar_metricas(self) -> dict:
        """Coleta métricas - roda na thread auxiliar (sem tocar em widgets)"""
    ...
    def _aplicar_metricas(self, dados: dict):
        """Aplica métricas nos labels - SEMPRE executado na thread principal"""
        try:
