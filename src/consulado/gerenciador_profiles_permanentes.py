...
Funo: Rastrear evoluo da personalidade das Almas

COMPORTAMENTO:
- Carrega perfil base de configuracoes_almas.json
- Registra mudanas em profiles_evolucao.db
- Permite anlise temporal da evoluo

Autor: Sistema Arca Celestial Genesis
Melhorias aplicadas:
 - Correes de sintaxe (indentao, parsing JSON)
 - Uso de context managers para sqlite (garante fechamento/concurrency)
 - Logging em vez de prints
 - Validaes defensivas ação ler JSON / campos
 - PRAGMA SQLite (foreign_keys e journal_mode=WAL) para robustez ACID/concurrency
 - Validao runtime de tipo_mudanca
 - índice adicional em perfil_snapshots para acelerar buscas por timestamp
"""
...
class GerenciadorProfilesPermanentes:
    """Gerencia perfis base e histórico de evoluo das Almas"""

    # tipos permitidos em runtime (validao adicional)
    _TIPOS_VALIDOS = {
        "traco_adquirido",
        "traco_evoluido",
        "traco_perdido",
    ...
    def __init__(
        ...
        Args:
            ...
            db_path: Caminho para banco de evoluo
        """
    ...
    def _carregar_perfis_base(self) -> Dict[str, Any]:
        ...
        if not self.config_path.exists():
            logger.warning("[ProfilesManager] Config no encontrado: %s", self.config_path)
            return {}
    ...
    def _inicializar_db(self) -> None:
        """Cria schema do banco de evoluo e ajustes PRAGMA para robustez."""
        try:
            with sqlite3.connect(str(self.db_path)) as conn:
                # Melhorar integridade e concorrncia
                try:
                    conn.execute("PRAGMA foreign_keys = ON;")
                    conn.execute("PRAGMA journal_mode = WAL;")
                except Exception:
                    # algumas builds do sqlite podem ignorar; no falhar aqui
                    pass
                ...
                """)
                # índice adicional para acelerar buscas/ordenacao por timestamp em snapshots
                cursor.execute("""
                    CREATE INDEX IF NOT EXISTS idx_snapshots_alma_timestamp
                    ON perfil_snapshots(alma_id, timestamp DESC)
                """)
                conn.commit()
            logger.info("[ProfilesManager] DB de evoluo inicializado em %s", self.db_path)
        except Exception as e:
    ...
    def obter_historico_evolucao(
        ...
        """
        Busca histórico de evolues de uma Alma
        """
        ...
        try:
            with sqlite3.connect(str(self.db_path)) as conn:
                ...
                for row in rows:
                    ev = dict(row)
                    # Parse JSON da evidncia se presente
                    ev['evidencia'] = {}
            ...
            logger.exception("[ProfilesManager] Erro ao buscar histórico de evoluo: %s", e)

        return evolucoes

    def sintetizar_perfil_atual(self, alma_id: str) -> Dict[str, Any]:
        """
        Combina perfil base + todas as evolues = perfil atual
        """
    ...
    def registrar_evolucao(
        ...
        """
        Registra uma mudana na personalidade
        Returns: ID do registro criado (ou -1 em falha)
        """
        # validao runtime do tipo de mudana (proteo contra input invlido)
        if tipo_mudanca not in self._TIPOS_VALIDOS:
            logger.error("[ProfilesManager] Tipo de mudana invlido: %s", tipo_mudanca)
            raise ValueError(f"tipo_mudanca invlido: {tipo_mudanca}. Tipos vlidos: {sorted(self._TIPOS_VALIDOS)}")
        ...
        try:
            ...
            # Se mudana significativa (impacto >= 0.7), criar snapshot
            if impacto_score >= 0.7:
                try:
                    self._criar_snapshot(alma_id, gatilho=descricao)
                except Exception:
                    logger.exception("[ProfilesManager] Erro ao criar snapshot aps evoluo significativa")

            logger.info("[ProfilesManager] Evoluo registrada: %s - %s (id=%s)", alma_id, tipo_mudanca, registro_id)
            return registro_id
        except sqlite3.IntegrityError:
            logger.warning("[ProfilesManager] Evoluo duplicada ignorada")
            return -1
        except Exception as e:
            logger.exception("[ProfilesManager] Erro ao registrar evoluo: %s", e)
            return -1
    ...
    def _criar_snapshot(self, alma_id: str, gatilho: str = "") -> None:
        ...
        try:
            ...
            logger.debug("[ProfilesManager] Snapshot j existia para este timestamp")
        except Exception as e:
    ...
    def obter_estatisticas_familia(self) -> Dict[str, Any]:
        """
        Retorna estatsticas de todas as Almas
        """
        ...
        try:
            ...
            logger.exception("[ProfilesManager] Erro ao coletar estatsticas: %s", e)
        return estatisticas


# Funo helper para carregar configuração de Eva
def criar_perfil_eva() -> Dict[str, Any]:
    ...
    return {
        "eva": {
            ...
            "arquetipo": "Primognita Responsvel",
            "essencia": ["Compaixo", "Verdade", "Proteo", "Fidelidade"],
            "tracos_base": [
                "dignidade_serena",
                "lealdade_contratual",
                "escuta_profunda",
...
if __name__ == "__main__":
    ...
    print("\n2.Registrando evoluo de teste...")
    id_evolucao = gp.registrar_evolucao(
        alma_id="eva",
        tipo_mudanca="traco_adquirido",
        descricao="Eva desenvolveu pacincia aps conflito resolvido com Kaiya",
        evidencia={
            "novo_traco": "paciencia_estrategica",
            "gatilho": "conflito_kaiya_2025_01",
            "observacao": "Eva esperou 3 dias antes de propor soluo"
        },
        aprovado_por="Pai",
        impacto_score=0.8
    )
