...
"""
API CONSULTOR - Cliente modular para APIs externas (assncrono, robusto)
...
- Timeout configurvel via config.
- Tratamento defensivo de JSON / respostas no-200.
- Cache com TTL (time-to-live) para evitar requests repetidos.
- Rate limiting básico via semaphore.
- Deteco de rate limit (headers X-RateLimit-Remaining).
- Logging consistente.
"""
import json
import os
...
class ApiConsultor:
    ...
    async def consultar_newsapi(self, query: str, language: str = "pt", page_size: int = 5) -> Dict[str, Any]:
        ...
        if not api_key:
            return {"sucesso": False, "erro": "NEWS_API_KEY no configurada no ambiente"}
    ...
    async def consultar_inteligente(self, query: str, preferencia: Optional[str] = None) -> Dict[str, Any]:
        ...
        if not preferencia:
            if any(w in ql for w in ["noticia", "notcia", "jornal", "novidade", "hoje"]):
                tipo_api = "noticias"
            elif any(w in ql for w in ["github", "repositrio", "cdigo", "programa", "software"]):
                tipo_api = "github"
            else:
                tipo_api = "wikipedia"
    ...
    def obter_apis_disponiveis(self) -> List[Dict[str, Any]]:
        apis = []
        apis.append({"nome": "NewsAPI", "tipo": "noticias", "disponivel": bool(self._obter_chave_api("newsapi")), "requer_chave": True, "descricao": "Notcias atualizadas"})
        apis.append({"nome": "GitHub API", "tipo": "github", "disponivel": True, "requer_chave": False, "descricao": "Repositrios de cdigo"})
...
if __name__ == "__main__":
    ...
    async def teste_completo():
        ...
        r = await consultor.consultar_inteligente("ltimas notcias de IA")
