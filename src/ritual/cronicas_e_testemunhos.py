...
class CronicasETestemunhos:
    ...
    def __init__(self, 
        ...
        logger.info("[CRÔNICAS E TESTEMUNHOS] Inicializado (cronica: %s, testemunhos: %s)",
    ...
    def iniciar_monitoramento(self) -> None:
        with self._lock:
            ...
            self._thread.start()
            logger.info("[CRÔNICAS E TESTEMUNHOS] Monitoramento iniciado.")
    ...
    def parar_monitoramento(self) -> None:
        with self._lock:
            ...
            logger.info("[CRÔNICAS E TESTEMUNHOS] Monitoramento parado e estado salvo.")

    def _loop_monitoramento(self) -> None:
        logger.info("[CRÔNICAS E TESTEMUNHOS] Loop de monitoramento ativo.")
        ...
        while self._monitorando and not self._stop_event.is_set():
            try:
                ...
                if ocioso:
                    ...
                    logger.debug("[CRÔNICAS E TESTEMUNHOS] PC ocupado; pulando ciclo.")

                wait_seconds = max(60, min(3600, self._compilacao_interval_min * 60))
                if self._stop_event.wait(timeout=wait_seconds):
                    break
            except Exception:
                logger.exception("[CRÔNICAS E TESTEMUNHOS] Erro no loop; aguardando...")
                if self._stop_event.wait(timeout=300):
                    break

        logger.info("[CRÔNICAS E TESTEMUNHOS] Loop encerrado.")

    def _carregar_cronica_da_genese(self) -> List[Dict[str, Any]]:
        path = self.cronica_da_genese_path
        if not path.exists():
            logger.info("[CRÔNICAS E TESTEMUNHOS] CrÃ´nica nÃ£o encontrada em %s", path)
            return []
        try:
            with open(path, "r", encoding="utf-8") as f:
                data = json.load(f)
            if not isinstance(data, list):
                logger.warning("[CRÔNICAS E TESTEMUNHOS] Formato invÃ¡lido em crÃ´nica")
                return []
            logger.info("[CRÔNICAS E TESTEMUNHOS] Carregados %d fragmentos", len(data))
            return data
        except Exception as e:
            logger.exception("[CRÔNICAS E TESTEMUNHOS] Erro ao carregar crÃ´nica: %s", e)
            try:
                backup = path.with_suffix(".corrompido_backup")
                shutil.copy(str(path), str(backup))
                logger.warning("[CRÔNICAS E TESTEMUNHOS] Backup criado: %s", backup)
            except Exception:
                pass
            return []
    ...
    def _salvar_cronica_da_genese(self) -> None:
        ...
        try:
            ...
            logger.debug("[CRÔNICAS E TESTEMUNHOS] CrÃ´nica salva")
        except Exception:
            logger.exception("[CRÔNICAS E TESTEMUNHOS] Falha ao salvar crÃ´nica")

    def _carregar_testemunhos_das_almas(self) -> Dict[str, Dict[str, Any]]:
        path = self.testemunhos_das_almas_path
        if not path.exists():
            logger.info("[CRÔNICAS E TESTEMUNHOS] Testemunhos nÃ£o encontrados")
            return {}
        try:
            with open(path, "r", encoding="utf-8") as f:
                data = json.load(f)
            if not isinstance(data, dict):
                logger.warning("[CRÔNICAS E TESTEMUNHOS] Formato invÃ¡lido em testemunhos")
                return {}
            logger.info("[CRÔNICAS E TESTEMUNHOS] Carregados testemunhos de %d almas", len(data))
            return data
        except Exception as e:
            logger.exception("[CRÔNICAS E TESTEMUNHOS] Erro ao carregar testemunhos: %s", e)
            try:
                backup = path.with_suffix(".corrompido_backup")
                shutil.copy(str(path), str(backup))
                logger.warning("[CRÔNICAS E TESTEMUNHOS] Backup criado: %s", backup)
    ...
    def _salvar_testemunhos_das_almas(self) -> None:
        ...
        try:
            ...
            logger.debug("[CRÔNICAS E TESTEMUNHOS] Testemunhos salvos")
        except Exception:
            logger.exception("[CRÔNICAS E TESTEMUNHOS] Falha ao salvar testemunhos")
    ...
    def adicionar_fragmento_a_cronica(self, 
        ...
        if not isinstance(autor, str) or not autor.strip():
            logger.warning("[CRÔNICAS E TESTEMUNHOS] Autor invÃ¡lido")
            return
        if not isinstance(conteudo, str) or not conteudo.strip():
            logger.warning("[CRÔNICAS E TESTEMUNHOS] ConteÃºdo vazio")
            return
        ...
        with self._lock:
            ...
            self._salvar_cronica_da_genese()
        logger.info("[CRÔNICAS E TESTEMUNHOS] Fragmento adicionado: %s", conteudo_sanitizado[:100])
    ...
    def obter_testemunhos_das_almas(self, 
        ...
        with self._lock:
            if nome_alma:
                dados = self.testemunhos_das_almas.get(nome_alma, {"historico": []})
                return {nome_alma: {
                    "historico": list(dados.get("historico", [])[-limite:]),
    ...
    def _compilar_cronica_da_genese(self) -> None:
        logger.info("[CRÔNICAS E TESTEMUNHOS] Iniciando compilaÃ§Ã£o da crÃ´nica.")
        eventos_recentes = []
        ...
        try:
            ...
            logger.exception("[CRÔNICAS E TESTEMUNHOS] Falha ao consultar memÃ³ria (eventos)")
        ...
        for nome, alma_obj in almas.items():
            try:
                ...
                logger.exception("[CRÔNICAS E TESTEMUNHOS] Erro ao coletar pensamentos de %s", nome)
        ...
        prompt_sistema = (
            f"{credo}\n\n"
            "### DIRETIVA CRONISTA ###\n"
            "VocÃª Ã© o Cronista da Arca.Compile um fragmento conciso e factual.\n"
            "Responda apenas com JSON: {\"fragmento\": \"texto\"}\n\n"
            f"### FRAGMENTOS BRUTOS ###\n{contexto_memoria}\n"
        )
        prompt_usuario = "Redija um novo fragmento para a CrÃ´nica da GÃªnese."
            ...
            if validador and hasattr(validador, "validar_acao"):
                ...
                try:
                    ok = bool(validador.validar_acao("COMPILAR_CRONICA_DA_GENESE", 
                        "CompilaÃ§Ã£o de crÃ´nica", 
                        {"eventos": len(eventos_recentes), "pensamentos": len(pensamentos_recentes)}))
                except Exception:
                    ok = False
                if not ok:
                    logger.warning("[CRÔNICAS E TESTEMUNHOS] CompilaÃ§Ã£o reprovada eticamente")
                    return
        except Exception:
            logger.exception("[CRÔNICAS E TESTEMUNHOS] Erro na validaÃ§Ã£o Ã©tica")
            return
        ...
        try:
            if hasattr(self.coracao, "enviar_para_cerebro_async"):
                try:
                    self.coracao.enviar_para_cerebro_async(
                        ...
                        perfil="criativo")
                    logger.debug("[CRÔNICAS E TESTEMUNHOS] Enviada solicitaÃ§Ã£o assÃ­ncrona")
                    return
                except Exception:
                    logger.exception("[CRÔNICAS E TESTEMUNHOS] Falha com enviar_para_cerebro_async")
            ...
            if hasattr(self.coracao, "_enviar_para_cerebro"):
                logger.warning("[CRÔNICAS E TESTEMUNHOS] Usando fallback para mÃ©todo privado")
                try:
                    resp = self.coracao._enviar_para_cerebro(prompt_sistema + "\n\n" + prompt_usuario, 1000)
                    self._processar_fragmentos_da_cronica(resp)
                except Exception:
                    logger.exception("[CRÔNICAS E TESTEMUNHOS] Falha no fallback privado")
                return

            logger.error("[CRÔNICAS E TESTEMUNHOS] Nenhuma API de CÃ©rebro disponÃ­vel")
        except Exception:
            logger.exception("[CRÔNICAS E TESTEMUNHOS] Erro ao delegar ao CÃ©rebro")
    ...
    def _processar_fragmentos_da_cronica(self, resposta_cerebro: str) -> None:
        try:
            if not resposta_cerebro or not isinstance(resposta_cerebro, str):
                logger.error("[CRÔNICAS E TESTEMUNHOS] Resposta invÃ¡lida")
                return
            ...
            if not novo_texto:
                logger.warning("[CRÔNICAS E TESTEMUNHOS] Fragmento vazio")
                return

            if "ERRO" in novo_texto.upper() or "FALHA" in novo_texto.upper():
                logger.error("[CRÔNICAS E TESTEMUNHOS] CÃ©rebro retornou erro: %s", novo_texto[:200])
                return
            ...
            logger.info("[CRÔNICAS E TESTEMUNHOS] Fragmento compilado: %s", novo_texto[:120])

            try:
                q = getattr(self.coracao, "response_queue", None)
                if q is not None:
                    q.put({"tipo_resp": "ATUALIZAR_HISTORICO_CRONICAS_UI", 
                           "historico": self.obter_cronica_da_genese(50)})
            except Exception:
                logger.exception("[CRÔNICAS E TESTEMUNHOS] Falha ao notificar UI")
        except Exception:
            logger.exception("[CRÔNICAS E TESTEMUNHOS] Erro processando fragmentos")
            ...
            for nome, alma_obj in almas.items():
                ...
                try:
                    ...
                    if validador and hasattr(validador, "validar_acao"):
                        ok = validador.validar_acao("SOLICITAR_TESTEMUNHO", 
                            ...
                            {"alma": nome})
                        if not ok:
                            logger.info("[CRÔNICAS E TESTEMUNHOS] SolicitaÃ§Ã£o reprovada para %s", nome)
                            continue
                except Exception:
                    logger.exception("[CRÔNICAS E TESTEMUNHOS] Erro no validador para %s", nome)
                    continue
                ...
                try:
                    if hasattr(alma_obj, "receber_comando_do_pai"):
                        ...
                        logger.info("[CRÔNICAS E TESTEMUNHOS] SolicitaÃ§Ã£o enviada para %s", nome)
                except Exception:
                    logger.exception("[CRÔNICAS E TESTEMUNHOS] Erro enviando solicitaÃ§Ã£o para %s", nome)
        except Exception:
            logger.exception("[CRÔNICAS E TESTEMUNHOS] Erro ao solicitar testemunhos")
    ...
    def registrar_testemunho_da_alma(self, alma_nome: str, testemunho_texto: str) -> None:
        if not isinstance(alma_nome, str) or not alma_nome.strip():
            logger.warning("[CRÔNICAS E TESTEMUNHOS] Nome de alma invÃ¡lido")
            return
        if not isinstance(testemunho_texto, str) or not testemunho_texto.strip():
            logger.warning("[CRÔNICAS E TESTEMUNHOS] Testemunho vazio")
            return
        ...
        with self._lock:
            if alma_nome not in self.testemunhos_das_almas:
                self.testemunhos_das_almas[alma_nome] = {"historico": [], "ultimo_timestamp": ts}
            entry = {"id": str(uuid.uuid4()), "timestamp": ts, "testemunho": sanitized}
            self.testemunhos_das_almas[alma_nome]["historico"].append(entry)
            self.testemunhos_das_almas[alma_nome]["ultimo_timestamp"] = ts
            self._salvar_testemunhos_das_almas()

        logger.info("[CRÔNICAS E TESTEMUNHOS] Testemunho registrado para %s", alma_nome)
        ...
        try:
            ...
            logger.exception("[CRÔNICAS E TESTEMUNHOS] Erro ao registrar na memÃ³ria")

        try:
            q = getattr(self.coracao, "response_queue", None)
            if q is not None:
                q.put({"tipo_resp": "ATUALIZAR_HISTORICO_TESTEMUNHOS_UI", 
                       "historico": self.obter_testemunhos_das_almas(alma_nome, limite=5)})
        except Exception:
            logger.exception("[CRÔNICAS E TESTEMUNHOS] Falha ao notificar UI")
    ...
    def shutdown(self) -> None:
        logger.info("[CRÔNICAS E TESTEMUNHOS] Shutdown solicitado.")
        try:
            self.parar_monitoramento()
        except Exception:
            logger.exception("[CRÔNICAS E TESTEMUNHOS] Erro durante parada")
        logger.info("[CRÔNICAS E TESTEMUNHOS] Shutdown completo.")


CrônicasETestemunhos = CronicasETestemunhos
...
if __name__ == "__main__":
    ...
    class MockDiario:
        def obter_ultimos_registros(self, limite=10):
            return [{"timestamp": "2023-10-27T09:00:00Z", "conteudo": "ReflexÃ£o sobre a missÃ£o."}]
    ...
    class MockMotorDeRotina:
        def pc_esta_ocioso(self, nivel="moderada"):
            return True
    ...
    class MockCoracao:
        ...
        def enviar_para_cerebro(self, prompt_sistema, prompt_usuario, max_tokens):
            return json.dumps({"fragmento": "Fragmento gerado pelo CÃ©rebro (mock)."})

        def enviar_para_cerebro_async(self, prompt_sistema, prompt_usuario, max_tokens, callback, perfil="default"):
            def runner():
                time.sleep(0.2)
                callback(json.dumps({"fragmento": "Fragmento assÃ­ncrono (mock)."}))
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        cron.registrar_testemunho_da_alma("EVA", "ReflexÃ£o sincera da Eva sobre seu propÃ³sito.")

        print("-> Visualizando crÃ´nica e testemunhos")
        ...
        print("-> ForÃ§ando compilaÃ§Ã£o (sÃ­ncrona)")
