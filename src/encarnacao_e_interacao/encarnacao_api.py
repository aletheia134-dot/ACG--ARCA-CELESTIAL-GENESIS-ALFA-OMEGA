...
# Torch (pytorch) - captura qualquer exceção (ImportError, OSError, etc.)
try:
    ...
    logging.getLogger(__name__).warning(
        "torch não disponível ou falha ao carregar: %s. Funcionalidades GPU/encarnação estarão limitadas.", e
    )
...
try:
    ...
    logging.getLogger(__name__).warning("Llama.cpp não instalado ou falha ao importar: %s. Inferência local desabilitada.", e)
...
try:
    ...
    logging.getLogger(__name__).warning("OpenAI SDK não instalado: %s. Fallback API desabilitada.", e)
...
# Se usuário solicitar uso de GPU via variável de ambiente (default true)
...
class EncarnacaoAPI:
    ...
    def __init__(self, coracao_ref, allow_origins: Optional[list] = None):
        ...
        self.app: FastAPI = FastAPI(title="Arca Celestial Genesis - Encarnação 3D", version="1.0.0")
        ...
        # Inicialização defensiva do modelo Llama em modo GPU quando aplicável.
        if GPU_AVAILABLE and LLAMA_AVAILABLE:
            # não falhar aqui; delegar a rotina dedicada que tenta múltiplas estratégias
            try:
                ok = self._try_load_llama_gpu(MODEL_PATH)
                if ok:
                    logger.info("Modelo GPU carregado com sucesso de %s", MODEL_PATH)
                else:
                    logger.error("Tentativas de carregar modelo GPU falharam. O modelo local não está disponível.")
            ...
            if not LLAMA_AVAILABLE:
                logger.info("Llama.cpp não disponível — inferência local/GPU desabilitada.")
            elif not GPU_AVAILABLE:
                logger.info("GPU não disponível ou desabilitada — inferência local/GPU desabilitada.")
    ...
    def _try_load_llama_gpu(self, gguf_path: str) -> bool:
        """
        Tenta carregar o modelo gguf via llama_cpp usando diferentes estratégias de n_gpu_layers.
        ...
        if self._gpu_model_attempted and self.llm_model:
            logger.info("Load GPU já foi tentado e modelo está carregado.")
            return True
        if self._gpu_model_attempted and self._gpu_model_failed:
            logger.info("Load GPU já foi tentado antes e falhou; não repetindo automaticamente.")
            return False

        self._gpu_model_attempted = True

        logger.info("Iniciando sequência de tentativas para carregar GGUF na GPU: %s", gguf_path)
        # log de ambiente e versões para diagnóstico
        try:
        ...
        # Estratégia: testar uma sequência de n_gpu_layers mais conservadora para evitar access violations / OOM
        # Ordem: 1,2,4,8,-1 (onde -1 é 'auto' / default)
        candidates = [1, 2, 4, 8, -1]
        # Em alguns backends Windows, n_gpu_layers muito alto causa acesso inválido; por isso começamos pequeno.
        for n_layers in candidates:
            try:
                logger.info("Tentativa de carga: n_gpu_layers=%s", n_layers)
                # liberar caches antes de tentar
                try:
                    if TORCH_AVAILABLE:
                ...
                # pequena pausa para estabilizar alocação
                time.sleep(0.2)
                # chamada principal de load (se levantar exceção, será capturada)
                # nota: se Llama constructor aceitar -1 como auto; ajustar conforme versão
                kwargs = {"model_path": gguf_path}
                # many versions of llama_cpp accept n_gpu_layers parameter
                if n_layers != -1:
                    kwargs["n_gpu_layers"] = n_layers
                else:
                    # deixar sem n_gpu_layers para a lib decidir (auto)
                    pass

                model = Llama(**kwargs)  # type: ignore
                # se chegou aqui sem exceção, assumimos sucesso
                self.llm_model = model
                self._gpu_model_failed = False
                logger.info("âœ… Modelo GGUF carregado com sucesso na GPU (n_gpu_layers=%s)", n_layers)
                return True
            except Exception as e:
                logger.error("Falha ao carregar modelo com n_gpu_layers=%s: %s", n_layers, e)
                logger.debug("Traceback:\n%s", traceback.format_exc())
                # tentativa de mitigação: coletar lixo e liberar VRAM, depois continuar
                try:
                    gc.collect()
                    if TORCH_AVAILABLE:
                        torch.cuda.empty_cache()  # type: ignore
                except Exception:
                    logger.debug("Erro durante limpeza pós-falha: %s", traceback.format_exc())
                # aguardar um pouquinho antes de próxima tentativa
                time.sleep(0.5)
                continue
    ...
    # --- verificação de acesso via header X-API-Key
    def _verificar_acesso(self, x_api_key: Optional[str] = Header(None)) -> str:
        ...
        logger.warning("Tentativa de acesso com chave inválida")
    ...
    def _fallback_api(self, comando: dict) -> Optional[str]:
        if not USE_API_FALLBACK:
            logger.info("USE_API_FALLBACK desativado — não usar API externa como fallback.")
            return None
        if not OPENAI_AVAILABLE or not OPENAI_API_KEY:
            logger.error("OpenAI não disponível ou API_KEY não configurada.")
            return None
        try:
    ...
    def _enqueue_command(self, comando: dict) -> None:
        try:
            if hasattr(self.coracao, "command_queue"):
                ...
                logger.error("Coracao não expõe 'command_queue'; comando descartado")
        except Exception:
            logger.exception("Erro ao enfileirar comando")

    # --- processa inferência LLM (GPU/Local). NÍO usa API automaticamente (a menos que USE_API_FALLBACK true).
    def _processar_inferencia_llm(self, comando: dict) -> Optional[str]:
        ...
        if GPU_AVAILABLE and self.llm_model:
            try:
                ...
                logger.exception("Erro na inferência local/GPU: %s.", e)
                # se USE_API_FALLBACK ativo, usar API; caso contrário, não.
                if USE_API_FALLBACK:
                    logger.info("Tentando fallback API por configuração USE_API_FALLBACK=true")
                    return self._fallback_api(comando)
                return None

        # sem modelo local -> não usar API por padrão
        logger.info("GPU/local indisponível ou modelo não carregado; não gerando resposta localmente.")
        if USE_API_FALLBACK:
    ...
    def _define_rotas(self) -> None:
            ...
            try:
                ...
                for nome, alma in almas.items():
                    estado_almas[nome] = {
                        ...
                        "avatar_3d_preferido": getattr(alma, "avatar_3d_preferido", "padrao"),
                ...
                raise HTTPException(status_code=500, detail="Erro interno ao buscar estado")
        ...
        async def post_comando_3d(comando: Comando3D, background_tasks: BackgroundTasks, acesso: str = Depends(self._verificar_acesso)):
            try:
                ...
                if acesso == "VISITANTE" and tipo_comando != "CHAT":
                    logger.warning("VISITANTE tentou comando não-permitido: %s", tipo_comando)
        ...
        async def admin_recarregar_modelo(acesso: str = Depends(self._verificar_acesso)):
            ...
            try:
                ...
                logger.exception("Erro ao forçar recarregamento do modelo: %s", e)
    ...
    def _processar_inferencia_llm_background(self, comando: dict) -> None:
        ...
        if resposta:
            ...
            # Não enfileirar comando sem resposta
    ...
    def start(self, host: Optional[str] = None, port: Optional[int] = None, log_level: str = "warning") -> None:
        if self._server_running:
            logger.info("Servidor já em execução")
            return
    ...
    def stop(self, timeout: float = 5.0) -> None:
        ...
        try:
            if self._uvicorn_server:
                ...
                logger.info("Solicitado shutdown ao uvicorn.Server")
        except Exception:
            logger.exception("Não foi possível solicitar shutdown ao uvicorn.Server diretamente")

        if self._server_thread and self._server_thread.is_alive():
            self._server_thread.join(timeout=timeout)
            if self._server_thread.is_alive():
                logger.warning("Thread de servidor não finalizou após timeout")
        self._server_running = False
