...
# -*- coding: utf-8 -*-
"""
Motor de ExpressÍÂ£o / SÍÂ­ntese de Voz - ORQUESTRADOR DE SENTIDOS (enduricido)

Local: src/modules/motor_expressao.py

Melhorias aplicadas:
 - Getter de configuraÍÂ§ÍÂ£o tolerante (get_safe)
 - Prioridade da fila robusta: enfileira tuplas (prioridade, contador, comando)
 - Contador monotÍÂ´nico para desempate; protegido por lock
 - Uso de threading.Event para parada reativa do loop de processamento
 - Chamadas a voz_real defensivas com tratamento de exceÍÂ§ÍÂµes
 - Logging seguro (preview + hash) para evitar vazamento de conteÍÂºdo
 - _load_config_values() chamado no __init__
 - Timeouts e comportamentos defensivos ao operar fila e threads
"""
...
# --- IMPORTAÍâ€¡ÕES MODULARES RIGOROSAS ---
try:
    ...
    logging.getLogger("MotorExpressao").critical(f"ERRO CRÍÂTICO: DependÍÂªncia Sentidos Humanos ausente: {e}")
    raise
...
class ComandoExpressao:
    ...
    # Nota: a ordenaÍÂ§ÍÂ£o Í© feita externamente via tupla (prioridade, contador, comando)
...
class MotorExpressao:
    ...
    def _enqueue(self, comando: ComandoExpressao):
        ...
        with self._counter_lock:
            ...
            # menor valor de prioridade => executa antes (se desejar o inverso, inverta a lÍ³gica)
    ...
    def expressar(self, comando: ComandoExpressao):
        ...
        # inicia processamento se necessÍÂ¡rio
    ...
    def iniciar_processamento(self):
        ...
        self.logger.info("Processamento de expressÍÂ£o iniciado.")

    def _processar_fila(self):
        # Loop responsivo ao stop_event
        try:
            while not self._stop_event.is_set():
                ...
                try:
                    ...
                    self.logger.exception("Erro ao executar expressÍÂ£o (continuando)")
        finally:
            self._running = False
            self.logger.info("Processamento de expressÍÂ£o finalizado.")
    ...
    def _executar_expressao(self, comando: ComandoExpressao):
        if comando.tipo == TipoExpressao.FALA:
            ...
            diagnostico = {}
            try:
                # chamada defensiva ao motor de voz real
                if hasattr(self.voz_real, "falar_real"):
                    try:
                        ...
                        if isinstance(res, tuple) and len(res) >= 1:
                            ...
                            # caso nÍÂ£o esperado, considerar sucesso se truthy
                            sucesso = bool(res)
                    except Exception:
                        self.logger.exception("Exception ao chamar voz_real.falar_real")
                        sucesso = False
                        diagnostico = {"erro": "excecao_chamada"}
                else:
                    self.logger.error("voz_real nÍÂ£o implementa 'falar_real'")
                    sucesso = False
            ...
            # integraÍÂ§ÍÂ£o com avatar 2D se houver (defensiva)
            self.logger.info("Mostrar avatar 2D para %s: %s", comando.alma_nome, comando.conteudo[:80])
            # implementaÍÂ§ÍÂ£o especÍÂ­fica do avatar nÍÂ£o fornecida aqui
        else:
            self.logger.warning("Tipo de expressÍÂ£o desconhecido: %s", comando.tipo)
